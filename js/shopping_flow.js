 ...
 * 改变配送方式 by wang
 */
function changeShipping(obj,seller_id)
{

  var index = obj.selectedIndex; // 选中索引
  var supportCod = obj.options[index].attributes['supportCod'].value + 0;
  var theForm = obj.form;
  ...
  var insure_id='ECS_NEEDINSURE_'+seller_id;
  if (obj.options[index].attributes['insure'].value + 0 == 0)
  {
    document.getElementById(insure_id).checked = false;
    document.getElementById(insure_id).disabled = true;
  }
  else
  {
    document.getElementById(insure_id).checked = false;
    document.getElementById(insure_id).disabled = false;
  }

  var now = new Date();
  Ajax.call('flow.php?step=select_shipping', 'shipping=' + obj.value+'&seller_id='+seller_id, orderShippingSelectedResponse, 'GET', 'JSON');
}
...
function check_OrderForm(frm)
{
  ...
  if ( ! paymentSelected)
  {
   ...
   var ship_obj=document.getElementsByName('shipping');
  for(k in ship_obj)
  {
	     var index = ship_obj[k].selectedIndex; // 选中索引
  		 if(ship_obj[k].options[index].value==0)
		 {
 ...
 * 选定了配送保价
 */
function select_Insure(needInsure,seller_id)
{
  needInsure = needInsure ? 1 : 0;

  Ajax.call('flow.php?step=select_insure', 'insure=' + needInsure+'&seller_id='+seller_id, orderSelectedResponse, 'GET', 'JSON');
}
