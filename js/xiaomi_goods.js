$(function(){
	...
	var goods_discuss = $("#goods_discuss").offset().top - 65;
	...
	$(window).scroll(function(){
		if ($(window).scrollTop() > $(".AreaR").offset().top + 52){
			...
			if($(window).scrollTop() >= goods_discuss)
			{
				$("#goddsDescMenu li:not(:eq(4))").removeClass("current");
				$("#goodsSubBar .clearfix li:not(:eq(4))").removeClass("current");
				$("#goddsDescMenu li:eq(4)").addClass("current");
				$("#goodsSubBar .clearfix li:eq(4)").addClass("current");
			}
		}
		else
		{
