...
var category = {
	...
	TPL_Simple: '{for item in data}		<div class="item fore${parseInt(item_index)}">			<span clstag="homepage|keycount|home2013|06{if parseInt(item_index)+1>9}${parseInt(item_index)+1}{else}0${parseInt(item_index)+1}{/if}a"><h3>			{for sItem in item}{if sItem_index!=0}\u3001{/if}<a href="${sItem.l}">${sItem.n}</a>{/for}			</h3><s></s></span>		</div>		{/for}<div class="extra"><a href="http://www.jd.com/allSort.aspx">\u5168\u90e8\u5546\u54c1\u5206\u7c7b</a></div>',
	...
	TPL_Items: '{for item in data}		<div class="item fore${parseInt(item_index)+1}">			<span clstag="homepage|keycount|home2013|0${601+parseInt(item_index)}a"><h3>${item.n}</h3><s></s></span>			<div class="i-mc">				<div onclick="$(this).parent().parent().removeClass(\'hover\')" class="close"></div>				<div class="subitem" clstag="homepage|keycount|home2013|0${601+parseInt(item_index)}b">					{for subitem in item.i}					<dl class="fore${parseInt(subitem_index)+1}">						<dt>							${category.FN_GetLink(1,subitem)}						</dt>						<dd>{for link in subitem.i}<em>${category.FN_GetLink(2,link)}</em>{/for}</dd>					</dl>					{/for}				</div>				<div class="cat-right-con fr" id="JD_sort_${item.t}"><div class="loading-style1"><b></b>\u52a0\u8f7d\u4e2d\uff0c\u8bf7\u7a0d\u5019...</div></div>			</div>		</div>		{/for}<div class="extra"><a clstag="homepage|keycount|home2013|0614a" href="http://www.jd.com/allSort.aspx">\u5168\u90e8\u5546\u54c1\u5206\u7c7b</a></div>',
	...
	getDataService: function(b) {
		...
		function(e) {
			...
			d, g, f = $("#nav-2013").offset().top + 39;
			if (c <= f) {
				if (e.hasClass("fore13")) {
					g = 23
				} else {
					g = 3
				}
...
pageConfig.FN_InitContrast = function(d, c, b) {
	var d = d || "_contrast",
	b = b || "list",
	c = c || "http://misc.360buyimg.com/contrast/js/contrast.js?ver=" + ( + new Date),
