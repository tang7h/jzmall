...
else
{
    ...
    $sql   = "SELECT COUNT(*) FROM " .$ecs->table('goods'). " AS g ".
        "WHERE g.is_delete = 0 and g.is_display=1 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 $attr_in ".
    ...
    $sql = "SELECT g.goods_id, g.goods_name, g.market_price, g.is_new,g.comments_number,g.sales_volume, g.is_best, g.is_hot, g.shop_price AS org_price, ".
            ...
            "WHERE g.is_delete = 0 AND g.is_on_sale = 1 and g.is_display=1 AND g.is_alone_sale = 1 $attr_in ".
                "AND (( 1 " . $categories . $keywords . $brand . $min_price . $max_price . $intro . $outstock . " ) ".$tag_where." ) " .$where.
            "ORDER BY $sort $order";
			
	//wang		
    $arr=get_search_goods($sql,$size,$page);
	//根据区域商家排序商品 by wang
	if(isset($GLOBALS['ip_seller_ids'])&&!empty($GLOBALS['ip_seller_ids']))
	{
		if(count($arr)<$size)
		{
			...
			$no_ip_where.=" and g.seller_id not ".db_create_in($GLOBALS['ip_seller_ids'])." ";
			
			//去除和ip地区商品在一页的商品
			if(!empty($_SESSION['sd_goods_id']))
			{
				$no_ip_where.=" and g.goods_id not ".db_create_in($_SESSION['sd_goods_id'])." ";
			}
			...
			$sql = "SELECT g.goods_id, g.goods_name, g.market_price, g.is_new,g.comments_number,g.sales_volume, g.is_best, g.is_hot, g.shop_price AS org_price, ".
					...
					"LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
							"ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
					"WHERE g.is_delete = 0 AND g.is_on_sale = 1 and g.is_display=1 AND g.is_alone_sale = 1 $attr_in ".
						"AND (( 1 " . $categories . $keywords . $brand . $min_price . $max_price . $intro . $outstock . " ) ".$tag_where." ) " .$no_ip_where.
					"ORDER BY $sort $order";
					
			
			
			$no_ip_arr=get_search_goods($sql,$size,$no_ip_page);
			
			$d_size=$size-count($arr);

			
			$sd_goods=array_slice($no_ip_arr,0,$d_size);

			if(empty($_SESSION['sd_goods_id']))
			{
				$sd_goods_id=array();
				foreach($sd_goods as $key=>$val)
				{
					$sd_goods_id[$key]=$val['goods_id'];
					
				}
				$_SESSION['sd_goods_id']=$d_goods_id;
			}
			
			if(count($arr)>0)
			{
				$arr=array_merge_recursive($arr,$sd_goods);
			}
			else
			{
				$arr=$no_ip_arr;
...
//wang 根据ip区域获得商品
function get_search_goods($sql,$size,$page)
{
    ...
    while ($row = $GLOBALS['db']->FetchRow($res))
    {
        ...
        if($display == 'grid')
        {
            //@Author guan 关键字高亮显示 start
				$goods_name = sub_str($row['goods_name'], $GLOBALS['_CFG']['goods_name_length']);
				$goods_name = '<text>' . $goods_name . '</text>';
				foreach($arr_keyword as $key => $val_keyword)
				{
					//$goods_name = preg_replace("/($val_keyword)/i","<font style='color:red'>$1</font>", $goods_name);
					//$goods_name = str_replace("$val_keyword","<font style='color:red'>$val_keyword</font>", $goods_name);
					$goods_name = preg_replace("/(>.*)($val_keyword)(.*<)/Ui","$1<font>$val_keyword</font>\$3", $goods_name);
					//echo $val_keyword;
					//echo '<br />';
					//echo $goods_name;
					//echo '<br />';
				...
				//exit;
				$arr[$row['goods_id']]['goods_name']    = $GLOBALS['_CFG']['goods_name_length'] > 0 ?  $goods_name  : $goods_name;
				//模版页面样式错误，为模版页面的的goods_name改为goods_name2。以防止样式错误。
				$arr[$row['goods_id']]['goods_name2']    = $GLOBALS['_CFG']['goods_name_length'] > 0 ?  $row['goods_name']  : $row['goods_name'];
				 //@Author guan 关键字高亮显示 end
        }
        else
        {
             //@Author guan 关键字高亮显示 start
				$goods_name = '<text>' . $row['goods_name'] . '</text>';
				foreach($arr_keyword as $key => $val_keyword)
				{
					//$goods_name = preg_replace("/($val_keyword)/i","<font style='color:red'>$1</font>", $goods_name);
					$goods_name = preg_replace("/(>.*)($val_keyword)(.*<)/Ui","$1<font>$val_keyword</font>\$3", $goods_name);
				}
				$arr[$row['goods_id']]['goods_name'] = $goods_name;
				$arr[$row['goods_id']]['goods_name2'] = $row['goods_name'];
				//@Author guan 关键字高亮显示 end
        }
