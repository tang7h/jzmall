    ...
    if ($GLOBALS['db']->getOne($sql) || $parent_id == 0)
    {
        ...
        foreach ($res AS $row)
        {
			...
			foreach ($brands AS $key => $val)
			{
				...
				$brands[$key]['url'] = build_uri('brand', array('cid' => $cat_id, 'bid' => $val['brand_id']));
...
function get_promote_goods($cats = '')
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
            ...
            "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
            'WHERE g.is_on_sale = 1 and g.is_display=1 AND g.is_alone_sale = 1 AND g.is_delete = 0 ' .
...
function get_category_recommend_goods($type = '', $cats = '', $brand = 0, $min =0,  $max = 0, $ext='', $num = '')
{
    ...
    $sql =  'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
            ...
            "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                    "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
            'WHERE g.is_on_sale = 1 and g.is_display=1 AND g.is_alone_sale = 1 AND g.is_delete = 0 ' . $brand_where . $price_where . $ext;$type2lib = array('best'=>'recommend_best', 'new'=>'recommend_new', 'hot'=>'recommend_hot', 'promote'=>'recommend_promotion');
     if($num != 'hot_single')
    {
    	$num = 0;
...
function get_goods_info($goods_id)
{
    ...
    if ($row !== false)
    {
		...
		$row['com_count']=com_count($goods_id);
		...
		if($row['is_promote'] == 1){
			...
			$row['promote_price'] = str_replace(array('￥','元'),'',price_format($promote_price));
			$row['promote_end_date'] =  $row['promote_end_date'];  // 活动结束时间
		}else{
		...
		$row['sheng_price'] = $row['market_price']-$shop_price;   //ecmoban模板堂 --zhuo节省价格
		$row['zhekou_price'] = round(($row['promote_price']/$row['market_price'])*10,1)+""; //ecmoban模板堂 --zhuo改---折扣显示
		//ecmoban模板堂 --zhuo end
    ...
    if (!empty($attr))
    {
        foreach ($attr['lnk'] AS $key => $val)
        {
            ...
            $sql = 'SELECT g.goods_id, g.goods_name, g.goods_thumb, g.goods_img, g.shop_price AS org_price, ' .
                    ...
                    "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                        "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
                    "WHERE a.attr_id = '$key' AND g.is_on_sale=1 and g.is_display=1  AND a.attr_value = '$val[value]' AND g.goods_id <> '$_REQUEST[id]' " .
	...
	foreach($cat_list_arr as $key=>$value)
	{
		if($value['level'] == 1)
		{
			 $sql = 'SELECT g.goods_id,g.cat_id, g.goods_name, g.market_price, g.shop_price AS org_price, ' .
					...
					'LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp '.
					"ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".					
					'WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 and g.is_display=1  AND is_delete = 0 AND ' . get_children($value['cat_id']) . ' ORDER BY g.sort_order, g.goods_id DESC';
			if ($num > 0)
			{
				$sql .= ' LIMIT ' . $num;
			}
 ...
 * @access  public
 * @param   mix     $spec   规格ID的数组或者逗号分隔的字符串
 * @return  void  //ecmoban模板堂 --zhuo
 */
function spec_price_group($spec)
{
    if (!empty($spec))
    {
        $where = db_create_in($spec, 'goods_attr_id');
		...
		$sql = 'SELECT attr_value FROM '. $GLOBALS['ecs']->table('goods_attr') . " WHERE $where";
		$res = $GLOBALS['db']->getAll($sql);
		$attr_value = '';
		foreach($res as $key=>$value)
		{
			...
			$attr_value .=$fh.'"'.$value['attr_value'].'"';	
		}
		$arr['attr_price'] = $price;
  ...
  // $val = serialize( $arr );
   $res = array();
        foreach ( $arr as  $key ){
            ...
            $res[] = array('attr_name' => $key );
            
        }
//    foreach( $arr as $key => $value ){
//        
//        $sql = " SELECT attr_value FROM " . $GLOBALS['ecs']-> table('goods_attr') . " WHERE goods_attr_id = " .$value;
...
/**
 * 获得已选定商品属性的商品名称  by Leah
 */
function get_attr_info($arr, $goods_id , $rec_id ){
    ...
    if (!empty($arr))
    {
        ...
        $goods_info = $GLOBALS['db'] ->getRow( $sql );   //商品名称
        
        $fmt = "%s:%s[%s] \n";

        $sql = "SELECT a.attr_name , ga.goods_attr_id , ga.attr_value, ga.attr_price ".
        ...
        while ($row = $GLOBALS['db']->fetchRow($res))
        {
            $attr_price = round(floatval($row['attr_price']), 2);
                ...
                $attr .= '<img src='.$row['attr_thumb'].'><span class="goods_name">'.$goods_info['goods_name'].'</span></br>'; 
            }
            
            $attr_price1 += $row['attr_price'];
            
            $attr .= '<input type="hidden" name="attr_val[]" value='.$row['goods_attr_id'].'><span>'.sprintf($fmt, $row['attr_name'], $row['attr_value'], $attr_price).'</span>';
        
            $array .=  sprintf($fmt, $row['attr_name'], $row['attr_value'], $attr_price);
            
        }
       
            
            $attr .= '<input type="hidden" name="array_attr[]" value="'.$array.'"/>';
  
        $price = price_format($goods_info['shop_price']+$attr_price1 );
        $attr .= '<input type="hidden" name="attr_num[]" value="1"><span>数量：1</span><span>单价:'.$price.'</span><span><a href="#" class="red" onclick="deleteNum(this)">删除</a></span>';
...
function get_on_top_goods($id ,$num = 5)
{
	...
	foreach($res as $idx=>$row)
	{
		...
		//获取符合ip地区商品的数量
		$ip_count=$GLOBALS['db']->getOne("select count(goods_id) from ".$GLOBALS['ecs']->table('goods')." as g WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 and g.is_display=1 AND 
                g.is_delete = 0 AND (" . $children .  ") ".$where);
		 
		 $sql = 'SELECT g.goods_id,g.seller_id, g.goods_name, g.market_price, g.shop_price AS org_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
               'g.promote_price, promote_start_date, promote_end_date, g.goods_brief, g.goods_thumb, g.goods_img ' .
            "FROM " . $GLOBALS['ecs']->table('goods') . ' AS g '.
            "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
			...
			$no_ip_goods_res=array();
			if($ip_count<$num)
			{
		 
				$no_ip_num=$num-$ip_count;
				//wang 根据ip地区商家搜索商品
				if(isset($GLOBALS['ip_seller_ids'])&&!empty($GLOBALS['ip_seller_ids']))
				{
				 ...
				 $sql = 'SELECT g.goods_id, g.goods_name, g.market_price, g.shop_price AS org_price, ' .
					...
					$no_ip_goods_res = $GLOBALS['db']->getAll($sql);
			}
			
			$goods_res=array_merge_recursive($goods_res,$no_ip_goods_res);
...
{
     $sql= "select count(*) as count from ".$GLOBALS['ecs']->table('comment')." where id_value='".$goods_id."' AND status!=0";
...
function getBrand($cat_id,$num = 20){
    $cat_id = $cat_id;
    $children = get_children($cat_id);
    /* 获取分类下品牌 */
    $sql = "SELECT b.brand_id, b.brand_logo,b.brand_name, COUNT(*) AS goods_num ".
        ...
        "WHERE g.brand_id = b.brand_id AND ($children OR " . 'gc.cat_id ' . db_create_in(array_unique(array_merge(array($cat_id), array_keys(cat_list($cat_id, 0, false))))) . ") AND b.is_show = 1 " .
        " AND g.is_on_sale = 1 and g.is_display=1  AND g.is_alone_sale = 1 AND g.is_delete = 0 ".
        "GROUP BY b.brand_id HAVING goods_num > 0 ORDER BY b.sort_order, b.brand_id ASC limit 1,{$num}";
 ...
 * @return  array
 */
function get_brand_list_relation($goods_id)
{
    $cat_id=$GLOBALS['db']->GetOne("select cat_id from ".$GLOBALS['ecs']->table('goods')." where goods_id='$goods_id'");

    $sql = "SELECT DISTINCT b.brand_name, b.brand_id FROM " . $GLOBALS['ecs']->table('brand') . "AS  b " .
        "LEFT JOIN " . $GLOBALS['ecs']->table('goods') . " AS g ON g.brand_id = b.brand_id " .
        "WHERE  g.cat_id = '$cat_id' AND g.is_alone_sale = 1 AND g.is_delete = 0  AND goods_id!='$goods_id'".
        "LIMIT 5 ";
    $res = $GLOBALS['db']->getAll($sql);
