...
function insert_browse_related()
{
	...
	if (!empty($_COOKIE['ECS']['history']))
    {
		...
		$sql   = 'SELECT cat_id FROM ' . $GLOBALS['ecs']->table('goods') .
                " WHERE $where AND is_on_sale = 1 and  is_display=1  AND is_alone_sale = 1 AND is_delete = 0";
		...
		if(!empty($arr[0]))
		{
			...
			$sql = 'SELECT g.goods_id,g.cat_id, g.goods_name, g.market_price, g.shop_price AS org_price, ' .
				...
				'WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 and g.is_display=1  and g.is_display=1  AND '.
...
function insert_history()
{
    ...
    if (!empty($_COOKIE['ECS']['history']))
    {
        ...
        $sql   = 'SELECT goods_id, promote_price, goods_name, goods_thumb, shop_price FROM ' . $GLOBALS['ecs']->table('goods') .
                " WHERE $where AND is_on_sale = 1 AND is_alone_sale = 1 and is_display=1  AND is_delete = 0";
...
/**
 * 调用会员信息  //ecmoban模板堂 --zhuo
 *
 * @access  public
 * @return  string
 */
function insert_member_info_group()
{
    $need_cache = $GLOBALS['smarty']->caching;
    $GLOBALS['smarty']->caching = false;
    ...
    $output = $GLOBALS['smarty']->fetch('library/member_info_group.lbi');
...
/**
 * 调用团购评论信息  ecmoban模板堂 --zhuo GROUP
 *
 * @access  public
 * @return  string
 */
function insert_comments_group($arr)
{
    $need_cache = $GLOBALS['smarty']->caching;
    $need_compile = $GLOBALS['smarty']->force_compile;
    ...
    $cmt = assign_comment($arr['id'],          $arr['type'],1,4);
    $GLOBALS['smarty']->assign('comments',     $cmt['comments']);
    $GLOBALS['smarty']->assign('pager',        $cmt['pager']);
	
    $val = $GLOBALS['smarty']->fetch('library/comments_list_group.lbi');
...
/**
 * 调用团购评论信息  ecmoban模板堂 --zhuo GOOD
 *
 * @access  public
 * @return  string
 */
function insert_comments_group_good($arr)
{
    $need_cache = $GLOBALS['smarty']->caching;
    $need_compile = $GLOBALS['smarty']->force_compile;
    ...
    $cmt_good = assign_comment($arr['id'],          $arr['type'],1,1);
    $GLOBALS['smarty']->assign('comments_o',     $cmt_good['comments']);
    $GLOBALS['smarty']->assign('pager_o',        $cmt_good['pager']);
	
    $val = $GLOBALS['smarty']->fetch('library/comments_list_group_good.lbi');
...
/**
 * 调用团购评论信息  ecmoban模板堂 --zhuo MIDDLE
 *
 * @access  public
 * @return  string
 */
function insert_comments_group_middle($arr)
{
    $need_cache = $GLOBALS['smarty']->caching;
    $need_compile = $GLOBALS['smarty']->force_compile;
    ...
    $cmt_middle = assign_comment($arr['id'],          $arr['type'],1,2);
	//echo "<pre>";
	//print_r($cmt_middle);
	
    $GLOBALS['smarty']->assign('comments_t',     $cmt_middle['comments']);
    $GLOBALS['smarty']->assign('pager_t',        $cmt_middle['pager']);
	
    $val = $GLOBALS['smarty']->fetch('library/comments_list_group_middle.lbi');
...
/**
 * 调用团购评论信息  ecmoban模板堂 --zhuo SHORT
 *
 * @access  public
 * @return  string
 */
function insert_comments_group_short($arr)
{
    $need_cache = $GLOBALS['smarty']->caching;
    $need_compile = $GLOBALS['smarty']->force_compile;
    ...
    $cmt_short = assign_comment($arr['id'],          $arr['type'],1,3);
    $GLOBALS['smarty']->assign('comments_th',     $cmt_short['comments']);
    $GLOBALS['smarty']->assign('pager_th',        $cmt_short['pager']);
	
    $val = $GLOBALS['smarty']->fetch('library/comments_list_group_short.lbi');
