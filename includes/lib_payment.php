...
function order_paid($log_id, $pay_status = PS_PAYED, $note = '')
{
    ...
    if ($log_id > 0)
    {
        ...
        if ($pay_log && $pay_log['is_paid'] == 0)
        {
			...
			/*--------用户白条------------*/
			//修改用户白条消费记录支付状态
			$bt_log_sql="update ".$GLOBALS['ecs']->table('baitiao_log')." set is_repay=1,repayed_date=".gmtime()." where order_id='$pay_log[order_id]'";
			$is_pay_bt=$GLOBALS['db']->query($bt_log_sql);
			/*--------用户白条------------*/
