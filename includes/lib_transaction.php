...
function get_profile($user_id)
{
    ...
    $info  = array();
    $infos = array();
    $sql  = "SELECT user_name, nick_name,birthday, sex, question, answer, rank_points, pay_points,user_money, user_rank,".
    ...
    $infos['nick_name'] = addslashes($infos['nick_name']);
 ...
 */
function add_bonus($user_id, $bouns_sn,&$errMsg='')
{
    if (empty($user_id))
    {
        $GLOBALS['err']->add($GLOBALS['_LANG']['not_login']);
        $errMsg = $GLOBALS['_LANG']['not_login'];
        return false;
    }
    ...
    if ($row)
    {
        if ($row['user_id'] == 0)
        {
            ...
            if ($now > $bonus_time['use_end_date'])
            {
                ...
                $errMsg = $GLOBALS['_LANG']['bonus_use_expire'];
                return false;
            }
        ...
        else
        {
            if ($row['user_id']== $user_id)
            {
                ...
                $errMsg = $GLOBALS['_LANG']['bonus_is_used'];
            }
            else
            {
                //红包被其他人使用过了。
                $GLOBALS['err']->add($GLOBALS['_LANG']['bonus_is_used_by_other']);
                $errMsg = $GLOBALS['_LANG']['bonus_is_used_by_other'];
            }
...
function get_user_orders($user_id, $num = 10, $start = 0)
{
    ...
    $sign = $GLOBALS['db']->getOne( $sql );      //无签收日可退货时间  

    $sql = 'SELECT value FROM ' . $GLOBALS['ecs']->table('shop_config').' WHERE code="sign_time"';
    $sign_time = $GLOBALS['db']->getOne( $sql );  // 有签收时间可退货时间
    $time = gmtime();
     /**by Leah E*/
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        else if ($row['order_status'] == OS_SPLITED)
        {
            ...
            {
               //已发货
                @$row['handler'] .= "<br/><a href=\"user.php?act=affirm_received&order_id=" .$row['order_id']. "\" onclick=\"if (!confirm('".$GLOBALS['_LANG']['confirm_received']."')) return false;\">".$GLOBALS['_LANG']['received']."</a>";
            /**by Leah S*/   
            //判断收货时间  是否可以退换货
                if( $row['sign_time']){
                    //有收货时间
                    $day = (($time-$row['sign_time'])/3600/24);
                    if( $day < $sign_time ){
                        
                        @$row['handler'] .= "<br/><a href=\"user.php?act=goods_order&order_id=" .$row['order_id']. '" style="margin-left:5px;" >' .$GLOBALS['_LANG']['return']."</a>";
                     ...
                     // 没有收货日期 取得发货时间
                     $sql = "SELECT log_time FROM " . $GLOBALS['ecs']->table('order_action') . " WHERE order_id =".$row['order_id']." and shipping_status=1";
                     $ship= $GLOBALS['db']->getOne($sql);
                     $day = (($time-$ship)/3600/24);
                     if( $day < $sign ){
                      @$row['handler'] .= "<br/><a href=\"user.php?act=goods_order&order_id=" .$row['order_id']. '" style="margin-left:5px;" >' .$GLOBALS['_LANG']['return']."</a>";  
                    }
            ...
            elseif ($row['shipping_status'] == SS_RECEIVED)
            {
                ...
                //已收货
                @$row['handler'] .= '<br/><span style="color:red">'.$GLOBALS['_LANG']['ss_received'] .'</span>';
                /**by Leah S*/
                //判断收货时间  是否可以退换货
                if( $row['sign_time']){
                    //有收货时间
            ...
            else
            {
                ...
                {
                    @$row['handler'] .= "<br/><a href=\"user.php?act=order_detail&order_id=" .$row['order_id']. '">' .$GLOBALS['_LANG']['pay_money']. '</a>';
                }
                else
                {
                    @$row['handler'] .= "<br/><a href=\"user.php?act=order_detail&order_id=" .$row['order_id']. '">' .$GLOBALS['_LANG']['view_order']. '</a>';
                }
        ...
        {
            $row['handler'] .= '<br/><span style="color:red">'.$GLOBALS['_LANG']['os'][$row['order_status']] .'</span>';
        }
...
function get_user_orders_goods($user_id, $num = 10, $start = 0, $order_id = 0)
{
	...
	while ($row = $GLOBALS['db']->fetchRow($res))
	{
		...
		if($row['shipping_status'] == SS_RECEIVED)
		{
			...
			$comment_rank = $GLOBALS['db']->getRow($sql);
			...
			if($row['is_single'] == 0)
			{
				$row['single_sun'] = '<a target=_blank style="color:blue;" href=user.php?act=single_sun&order_id=' .$row['order_id'] . '&goods_id=' . $row['goods_id'] . '>晒单</a>';
			}
			...
			elseif($row['is_single'] == 2)
			{
				...
				//晒单赠送的积分数
				$row['integ'] = "<font style='color:red;'>" . $single['integ'] . "积分</font>";
			}
			else
			{
			...
			$arr[] = array('order_id'       => $row['order_id'],
					...
					'goods_name_style'=>sub_str($row['goods_name'],36),
...
function drop_consignee($id)
{
    ...
    {
        $sql = "SELECT address_id FROM " .$GLOBALS['ecs']->table('user') . " WHERE user_id = '$uid'";
        $defaultAddId = $GLOBALS['db']->getOne($sql);

        $sql = "DELETE FROM " .$GLOBALS['ecs']->table('user_address') . " WHERE address_id = '$id'";
        $res = $GLOBALS['db']->query($sql);

        //如果删除的为默认地址，则默认添加一个默认地址
        if($defaultAddId == $id){
            $sql = "SELECT address_id FROM " .$GLOBALS['ecs']->table('user_address') . " WHERE user_id = '$uid'";
            $addressId = $GLOBALS['db']->getOne($sql);
            if($addressId>0){
                $sql = 'update ' .$GLOBALS['ecs']->table('user_address') . ' set address_id='.$addressId.' where user_id='.$uid;
                $res = $GLOBALS['db']->query($sql);
            }
 ...
 */
function get_order_detail($order_id, $user_id = 0,$is_bt=false)
{
    ...
    /* 如果是未付款状态，或者是白条订单,生成支付按钮 */
	$is_payment=$is_bt?$is_bt:($order['pay_status'] == PS_UNPAYED &&($order['order_status'] == OS_UNCONFIRMED ||$order['order_status'] == OS_CONFIRMED));
