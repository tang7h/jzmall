 ...
 * @return  array   配送方式数组
 */
function seller_available_shipping_list($region_id_list,$seller_id=0)
{
    $sql = 'SELECT s.shipping_id, s.shipping_code, s.shipping_name, ' .
                's.shipping_desc, s.insure, s.support_cod, a.configure ' .
            'FROM ' . $GLOBALS['ecs']->table('shipping') . ' AS s, ' .
                ...
                $GLOBALS['ecs']->table('area_region') . ' AS r ' .
            'WHERE r.region_id ' . db_create_in($region_id_list) .
            ' AND r.shipping_area_id = a.shipping_area_id AND a.shipping_id = s.shipping_id AND s.enabled = 1 and r.seller_id='.$seller_id.' ORDER BY s.shipping_order';
...
function return_order_info( $ret_id , $order_sn = ''){
    ...
    if ($ret_id > 0)
    {
        $sql = "SELECT r.* , g.goods_name ,g.goods_thumb, o.order_sn ,o.add_time ,  d.delivery_sn , d.update_time , d.how_oos ,d.shipping_fee, d.insure_fee , d.invoice_no,
    ...
    $order['address_detail'] = get_consignee_info( $order['country'] , $order['province'] , $order['city'] , $order['district']); 
 ...
 * @return type
 */
function get_consignee_info( $country, $province,$city , $district){
    
    if( $country){
        
        $sql = 'SELECT  region_name FROM ' . $GLOBALS['ecs']->table('region') .
            " WHERE region_id = '$country'";
        $address  = $GLOBALS['db']->getOne($sql ).'省'; 
    ...
    if( $province ){
        ...
        $address  .= $GLOBALS['db']->getOne($sql ).'市'; 
    }
    if( $city ){
        
        $sql = 'SELECT region_name FROM ' . $GLOBALS['ecs']->table('region') .
            " WHERE region_id = '$city'";
        $address  .= $GLOBALS['db']->getOne($sql ).'区'; 
    }
    if( $district ){
...
function return_order(){
    ...
    $sql = "SELECT ret_id , rec_id, goods_id , order_sn ,order_id , apply_time , should_return, return_status , refound_status " .
            " FROM " . $GLOBALS['ecs']->table('order_return') .
    ...
    { 
         ...
         * 退款
         */
        if( $row['refound_status'] == 0 ){
            //未退款
            @$row['reimburse_status'] .= "<span>" .$GLOBALS['_LANG']['no_refound']."</span>";
        }
        elseif( $row['refound_status'] == 1){
            //已退款
            @$row['reimburse_status'] .= "<span>" .$GLOBALS['_LANG']['refound']."</span>";
        }
 ...
 */
 //wang 商家入驻
function seller_order_fee($order, $goods, $consignee,$order_counts=1)
{
    /* 初始化订单的扩展code */
    if (!isset($order['extension_code']))
    {
 ...
 * @param   array     $filterGoods   类型：针对购物车里的指定商品获取信息
 * @param   boolean     $isGoodsInfo   类型：是否要商品的其他详情信息
 * @return  array   购物车商品数组
 */
function cart_goods($type = CART_GENERAL_GOODS,$filterGoods=array(),$isGoodsInfo=false)
{
    $goodsInfoSql1 = '';
    $goodsInfoSql2 = '';

    if($isGoodsInfo){
        $goodsInfoSql1 = ',g.seller_id,g.goods_thumb,g.goods_img,s.shop_name ';
        $goodsInfoSql2 =
            ' inner join '.$GLOBALS['ecs']->table('goods').' as g on g.goods_id=c.goods_id '
            .' left join '.$GLOBALS['ecs']->table('seller_shopinfo').' as s on g.seller_id=s.seller_id ';
    }

    $fileGoodsSql = '';
    if(!empty($filterGoods)){
        $fileGoodsSql = ' and c.goods_id '.db_create_in(array_values($filterGoods));
    }
    ...
    $sql = "SELECT c.rec_id, c.user_id, c.goods_id, c.goods_name, c.goods_sn, c.goods_number, " .
        ...
        " FROM " . $GLOBALS['ecs']->table('cart') .' as c '.
            ...
            "AND rec_type = '$type'".$fileGoodsSql;
    ...
    foreach ($arr as $key => $value)
    {
        ...
        $arr[$key]['goods_thumb']     = API_DATA('PHOTO', get_image_path($value['goods_id'], $value['goods_thumb'], true));;
        $arr[$key]['goods_img']     = API_DATA('PHOTO', get_image_path($value['goods_id'], $value['goods_img'], true));;
 ...
 * @return  array
 */
function seller_cart_weight_price($type = CART_GENERAL_GOODS,$seller_id=0)
{
    ...
    /* 计算超值礼包内商品的相关配送参数 */
    $sql = 'SELECT goods_id, goods_number, goods_price FROM ' . $GLOBALS['ecs']->table('cart') . " WHERE extension_code = 'package_buy' AND session_id = '" . SESS_ID . "' and seller_id='".$seller_id."'";
    ...
    $sql    = 'SELECT SUM(g.goods_weight * c.goods_number) AS weight, ' .
                ...
                "AND rec_type = '$type' AND g.is_shipping = 0 AND c.extension_code != 'package_buy' and c.seller_id='".$seller_id."'";
    $row = $GLOBALS['db']->getRow($sql);
 ...
 * 清空购物车
 * @param   int     $type   类型：默认普通商品
 * @param   array     $filterGoods   类型：取出的订单商品对应的进行过滤
 */
function clear_cart($type = CART_GENERAL_GOODS,$filterGoods=array())
{
    $filterGoodsSql = '';
    if(!empty($filterGoods)){
        $filterGoodsSql = ' and goods_id '.db_create_in(array_values($filterGoods));
    }

    $sql = "DELETE FROM " . $GLOBALS['ecs']->table('cart').
            " WHERE session_id = '" . SESS_ID . "' AND rec_type = '$type'".$filterGoodsSql;
    $GLOBALS['db']->query($sql);
}
 ...
 *
 * @access  public
 * @params array  $filterGoods 要取出的购物车商品
 * @return  array
 */
function get_cart_goods($filterGoods=array())
{
    /* 初始化 */
    $goods_list = array();
    $total = array(
    ...
    $filterGoodsSql = '';
    if(!empty($filterGoods)){
        $filterGoodsSql = ' and c.goods_id '.db_create_in(array_values($filterGoods));
    }

    /* 循环、统计 */
    $sql = "SELECT *, IF(parent_id, parent_id, goods_id) AS pid " .
            " FROM " . $GLOBALS['ecs']->table('cart') . " as c " .
            " WHERE session_id = '" . SESS_ID . "' AND rec_type = '" . CART_GENERAL_GOODS . "'" .$filterGoodsSql.
...
function get_consignee($user_id)
{
    ...
    else
    {
        ...
        if ($user_id > 0)
        {
			...
			// by wang
			$arr['province_name']=$GLOBALS['db']->getOne("select region_name from ".$GLOBALS['ecs']->table('region')." where region_id='".$arr['province']."'");
			$arr['city_name']=$GLOBALS['db']->getOne("select region_name from ".$GLOBALS['ecs']->table('region')." where region_id='".$arr['city']."'");
			$arr['district_name']=$GLOBALS['db']->getOne("select region_name from ".$GLOBALS['ecs']->table('region')." where region_id='".$arr['district']."'");
			//by wang
        }
 ...
 * @return  array
 */
function seller_flow_order_info()
{
...
function cart_sup_goods($type = CART_GENERAL_GOODS)
{
	...
	{
		$sql = "SELECT c.rec_id, c.user_id, c.goods_id, c.goods_name, c.goods_sn, c.goods_number, g.goods_thumb, sum(g.wheat_coin) as wheat_coin, " .
            "c.market_price, c.goods_price, c.goods_attr, c.is_real, c.extension_code, c.parent_id, c.is_gift, c.is_shipping, " .
            "c.goods_price * c.goods_number AS subtotal " .
            ...
            "AND c.rec_type = '$type' and c.seller_id='".$seller['seller_id']."'" . "GROUP BY c.rec_id, c.user_id, c.goods_id, c.goods_name, c.goods_sn, c.goods_number, g.goods_thumb"; //聚合函数查询修复 g
		$arr = $GLOBALS['db']->getAll($sql);
