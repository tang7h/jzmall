 ...
 * @return  array
 */
function get_shop_license($is_reg=0)
{
    // 取出网店 license
...
function license_activation($activation_code,$certificate_id)
{
    // 登录信息配置
    $certi['certi_app'] = ''; // 证书方法
    $certi['certificate_id'] = $certificate_id; // 网店证书ID
    $certi['oem_id']='';   	
    $certi['active_key'] = $activation_code;   
    // 取出网店 license
    $license = get_shop_license();

    $certi['certi_app'] = 'oem.do_active'; // 证书方法
    $certi['ac'] = make_shopex_activation_ac($certificate_id,$certi['oem_id']); // 网店验证字符串
    $request_arr = exchange_shop_license($certi, $license);
    return($request_arr);
...
function make_shopex_activation_ac($certificate_id,$oem_id){
    $ac= md5($certificate_id.OEM_KEY.$oem_id);
    return $ac;
}
