 ...
 * @return  array   $arr
 */
function get_collection_seller($user_id, $num = 10, $start = 0)
{
    $sql = 'SELECT c.rec_id,s.*  FROM ' . $GLOBALS['ecs']->table('collect_seller') . ' AS c' .
            " LEFT JOIN " . $GLOBALS['ecs']->table('seller_shopinfo') . " AS s ".
                "ON c.seller_id = s.id ".
            " WHERE c.user_id = '$user_id' ORDER BY c.rec_id DESC";
    $res = $GLOBALS['db'] -> selectLimit($sql, $num, $start);
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $seller_list[$row['id']]['url']           = build_uri('seller_store', array('sid'=>$row['id']), $row['shop_title']);
    }
...
function get_user_default($user_id)
{
    ...
    $sql = "SELECT pay_points, user_money, credit_line, last_login, is_validated,wheat_coin FROM " .$GLOBALS['ecs']->table('users'). " WHERE user_id = '$user_id'";
    $row = $GLOBALS['db']->getRow($sql);
    $info = array();
	...
	//by wang
	$info['wheat_coin']=$row['wheat_coin'];
	$info['wheat_coin_value']=price_format($row['wheat_coin']*$GLOBALS['_CFG']['wheat_coin'],false);
