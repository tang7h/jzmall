 ...
 * @return  void
 */
function show_message1($content, $links = '', $hrefs = '', $type = 'info', $auto_redirect = false)
{
    assign_template();
 ...
 * @return  array
 */
function assign_comment($id, $type, $page = 1,$is_commets = NULL)
{
    /* 取得评论列表 */
	if($is_commets == 1){
		$count = $GLOBALS['db']->getOne('SELECT COUNT(*) FROM ' .$GLOBALS['ecs']->table('comment').
           		" WHERE id_value = '$id' AND comment_type = '$type' AND status = 1 AND parent_id = 0 and comment_rank in(5,4)");
	}elseif($is_commets == 2){
		$count = $GLOBALS['db']->getOne('SELECT COUNT(*) FROM ' .$GLOBALS['ecs']->table('comment').
           		" WHERE id_value = '$id' AND comment_type = '$type' AND status = 1 AND parent_id = 0 and comment_rank  in(3,2)");
	}elseif($is_commets == 3){
		$count = $GLOBALS['db']->getOne('SELECT COUNT(*) FROM ' .$GLOBALS['ecs']->table('comment').
	...
	}elseif($is_commets == 2){ //中评
		$sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('comment') .
            	" WHERE id_value = '$id' AND comment_type = '$type' AND status = 1 AND parent_id = 0 and comment_rank in(3,2)".
            	' ORDER BY comment_id DESC';
	}elseif($is_commets == 3){  //差评
		$sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('comment') .
            	" WHERE id_value = '$id' AND comment_type = '$type' AND status = 1 AND parent_id = 0 and comment_rank = 1".
            	' ORDER BY comment_id DESC';
	}elseif($is_commets == 4){  //全部评论
		$sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('comment') .
            	" WHERE id_value = '$id' AND comment_type = '$type' AND status = 1 AND parent_id = 0".
            	' ORDER BY comment_id DESC';
	}else{  //全部评论
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		//ecmoban模板堂 --zhuo start
        $arr[$row['comment_id']]['advantages']        = $row['advantages']; //ecmoban模板堂 --zhuo
		$arr[$row['comment_id']]['defect']       	  = $row['defect'];		   //ecmoban模板堂 --zhuo
		$arr[$row['comment_id']]['rank_points']       = get_group_rank_name(get_group_rank_points($row['user_name']));		   //ecmoban模板堂 --zhuo
		$arr[$row['comment_id']]['order_time']  	  = local_date("Y-m-d",get_group_order_time(get_fine_user_id($row['user_name']),$id));
		//ecmoban模板堂 --zhuo end
	...
	//ecmoban模板堂 --zhuo start
	if($is_commets == 1){
		$pager['good_page']    = $page;
   		$pager['size']         = $size;
    	$pager['record_count'] = $count;
    	$pager['page_count']   = $page_count;
    	$pager['page_first']   = "javascript:gotoPage_good(1,$id,$type)";
    	$pager['page_prev']    = $page > 1 ? "javascript:gotoPage_good(" .($page-1). ",$id,$type)" : 'javascript:;';
    	$pager['page_next']    = $page < $page_count ? 'javascript:gotoPage_good(' .($page + 1) . ",$id,$type)" : 'javascript:;';
    	$pager['page_last']    = $page < $page_count ? 'javascript:gotoPage_good(' .$page_count. ",$id,$type)"  : 'javascript:;';
	}elseif($is_commets == 2){
		$pager['middle_page']         = $page;
   		$pager['size']         = $size;
    	$pager['record_count'] 	   = $count;
    	$pager['page_count']   = $page_count;
    	$pager['page_first']   = "javascript:gotoPage_middle(1,$id,$type)";
    	$pager['page_prev']    = $page > 1 ? "javascript:gotoPage_middle(" .($page-1). ",$id,$type)" : 'javascript:;';
    	$pager['page_next']    = $page < $page_count ? 'javascript:gotoPage_middle(' .($page + 1) . ",$id,$type)" : 'javascript:;';
    	$pager['page_last']    = $page < $page_count ? 'javascript:gotoPage_middle(' .$page_count. ",$id,$type)"  : 'javascript:;';
	}elseif($is_commets == 3){
		$pager['short_page']   = $page;
   		$pager['size']         = $size;
    	$pager['record_count'] = $count;
    	$pager['page_count']   = $page_count;
    	$pager['page_first']   = "javascript:gotoPage_short(1,$id,$type)";
    	$pager['page_prev']    = $page > 1 ? "javascript:gotoPage_short(" .($page-1). ",$id,$type)" : 'javascript:;';
    	$pager['page_next']    = $page < $page_count ? 'javascript:gotoPage_short(' .($page + 1) . ",$id,$type)" : 'javascript:;';
    	$pager['page_last']    = $page < $page_count ? 'javascript:gotoPage_short(' .$page_count. ",$id,$type)"  : 'javascript:;';
	}elseif($is_commets == 4){
		$pager['group_page']   = $page;
   		$pager['size']         = $size;
    	$pager['record_count'] = $count;
    	$pager['page_count']   = $page_count;
    	$pager['page_first']   = "javascript:gotoPage_group(1,$id,$type)";
    	$pager['page_prev']    = $page > 1 ? "javascript:gotoPage_group(" .($page-1). ",$id,$type)" : 'javascript:;';
    	$pager['page_next']    = $page < $page_count ? 'javascript:gotoPage_group(' .($page + 1) . ",$id,$type)" : 'javascript:;';
    	$pager['page_last']    = $page < $page_count ? 'javascript:gotoPage_group(' .$page_count. ",$id,$type)"  : 'javascript:;';
	}else{
		$pager['page']         = $page;
   		$pager['size']         = $size;
    	$pager['record_count'] = $count;
