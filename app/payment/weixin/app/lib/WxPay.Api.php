...
class WxPayApi
{
     ...
     *
     *  app 应用返回信息
     * @param WxPayAppData $inputObj
     *
     */
    public static function appOrderData($prepayId){
        //检测必填参数
        if(empty($prepayId)) {
            throw new WxPayException("缺少统一必填参数prepay_id！");
        }
        $inputObj = new WxPayAppData();
        $inputObj->SetAppid(WxPayConfig::APPID);//公众账号ID
        $inputObj->SetMch_id(WxPayConfig::MCHID);//商户号
        $inputObj->SetPrepay_id($prepayId);
