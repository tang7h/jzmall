...
class AppCaptcha
{
     ...
     * @return  bool
     */
    function check_word($word,$cacheKey='')
    {
        include_once('Memcache.php');
        $recorded =  AppMemcache::factory()->get(GZ_Api::$_KEY_CACHE_CAPTURE.$cacheKey);
        $given = '';
        if(empty($recorded)){
            return false;
        }
        
        AppMemcache::factory()->del(GZ_Api::$_KEY_CACHE_CAPTURE.$cacheKey);

        $given    = base64_encode($this->encrypts_word(strtoupper($word)));

        $isMatch = (preg_match("/$given/", $recorded));
        if(!$isMatch){
            return false;
        }
     ...
     * @return  mix
     */
    function generate_image($word = false,$cacheKey='')
    {
        ...
        /* 记录验证码到session */
        $this->record_word($word,$cacheKey);
     ...
     * @return  void
     */
    function record_word($word,$cacheKey='')
    {
        include_once('Memcache.php');
        AppMemcache::factory()->set(GZ_Api::$_KEY_CACHE_CAPTURE.$cacheKey,base64_encode($this->encrypts_word($word)));
    }
