...
switch ($tmp[0]) {
	...
	case 'list':
		...
		foreach ($smarty->_var['goods_list'] as $key => $value) {
			...
			$temp['goods_collected'] = $value['rec_id'];
            $rankPrices = get_user_rank_prices($temp['goods_id'], $value['shop_price']);    // 会员等级价格
            $temp['vip_prices'] = !empty($rankPrices[2])?$rankPrices[2]['price']:0;
            $data[] = $temp;
		}
...
function GZ_get_collection_goods($user_id, $num = 10, $start = 1, $rec_id = 0)
{
    ...
    $sql = 'SELECT g.original_img,g.goods_id, g.goods_name, g.market_price, g.shop_price, g.goods_thumb, g.goods_img, g.original_img, g.goods_brief, g.goods_type ,g.seller_id AS org_price, '.
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $goods_list[$row['goods_id']]['seller_id']      = $row['seller_id'];
        $goods_list[$row['goods_id']]['goods_name']    = $row['goods_name'];
        $goods_list[$row['goods_id']]['market_price']  =$row['market_price']; //price_format($row['market_price']);
        $goods_list[$row['goods_id']]['shop_price']    = $row['shop_price'];//price_format($row['shop_price']);
        $goods_list[$row['goods_id']]['promote_price'] = ($promote_price > 0) ? $promote_price: '';
