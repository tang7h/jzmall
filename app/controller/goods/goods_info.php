...
if (GZ_Api::$session['uid']) {
    ...
    if (($rec_id = $GLOBALS['db']->GetOne($sql)) > 0) {
       $data['goods_collected'] = $rec_id;
    }else {
        $data['goods_collected'] = 0;
    }
}
$shopInfo = getShopInfo($data['seller_id']);

$data = API_DATA('GOODS',$data);
$saleCount = get_goods_sales_count($goods_id);
$data['sale_count'] = $saleCount;

$data['shop_info'] = $shopInfo;

$relationBrands = get_brand_list_relation($goods_id);

$data['relation_brand'] = $relationBrands;
...
function getShopInfo($seller_id){
    $store_info = array(
        'seller_id'=>$seller_id,
        'shop_name'=>'自营',
        'shop_address'=>'无',
    ...
    if($seller_id)
    {
        $tempStore=$GLOBALS['db']->getRow("select * from ".$GLOBALS['ecs']->table('seller_shopinfo')." where seller_id='".$seller_id."'");
        if($tempStore)
        {
            $sql = 'SELECT region_name FROM ' . $GLOBALS['ecs']->table('region') .
                " WHERE region_id in(".$tempStore['province'].",".$tempStore['city'].") order by region_id Asc";
            $address=$GLOBALS['db']->getCol($sql);
            $tempStore['address']=$address[0].$address[1];
//            $store_info['shop_logo']=str_replace('../','./',$store['shop_logo']);
            ...
            $store_info['shop_address'] = $tempStore['address'];
            $store_info['shop_name'] = $tempStore['shop_name'];
        }
