...
class Search{
     ...
     * 搜索商品列表
     */
    public function searchGoods($filter_attr_str,$sort,$order){
        ...
        $cat_id = 90;
        $brand = 0;
        $filter_attr_str = '2.0.15.40.0';
        $price_min = 0;
        $price_max = 0;
        $searchFilter = getSearchAttr($cat_id,$brand,$price_min,$price_max,$filter_attr);

        $brandArr = $searchFilter['brand'];
        $brandArr = $searchFilter['price'];
        $brandArr = $searchFilter['ext'];
        ...
        $goods_list = category_get_goods2($children, $brand, $price_min, $price_max, $ext, $pageSize, $page, $sort, $order);
     ...
     * 获取搜索条件
     */
    public function getSearchAttr($cat_id,$brand,$price_min,$price_max,$filter_attr=array()){
//        $cat_id = 90;
//        $brand = 0;
//        $filter_attr_str = '2.0.15.40.0';
//        $price_min = 0;
//        $price_max = 0;
        ...
        $result = GZ_get_category_search($cat_id,$brand,$price_min,$price_max,$filter_attr);
        return $result;
    }
...
//$searchObj->getSearchAttr();
//exit;
// 如果用户登陆，处理收藏状态
$filter = _POST('filter', array());
$ex_attr_str = isset($_REQUEST['ext_filed']) ? htmlspecialchars(trim($_REQUEST['ext_filed'])) : '0';
...
if ($sort_by == 'is_hot') {
    ...
    $sort = 'shop_price';
    $order = 'ASC';
}else if($sort_by == 'sale_count_desc'){
...
//      $cat = GZ_get_cat_info($_REQUEST['category']);   // 获得分类的相关信息
//  }
  ...
  $brand      = $_REQUEST['brand']            ? " AND brand_id = '".$_REQUEST['brand']."'" : '';
  $seller      = $_REQUEST['seller_id']            ? " AND seller_id = '".$_REQUEST['seller_id']."'" : '';
  $outstock   = !empty($_REQUEST['outstock']) ? " AND g.goods_number > 0 "           : '';
  $isNew   = !empty($_REQUEST['is_new']) ? " AND g.is_new =1 "           : '';
  ...
  $sql   = "SELECT COUNT(*) FROM " .$ecs->table('goods'). " AS g ".
      ...
      "AND (( 1 " . $categories . $keywords . $brand .$seller. $isNew .$min_price . $max_price . $intro . $outstock ." ) ".$tag_where." )";
  $count = $db->getOne($sql);
  ...
  if (GZ_Api::$session['uid']) {
      $sql = "SELECT COUNT(*) FROM " .$GLOBALS['ecs']->table('collect_goods') .
          " WHERE user_id='$_SESSION[user_id]' AND goods_id = '".$info['goods_id']."'";
     $isCollectSql = 'left join ' .$GLOBALS['ecs']->table('collect_goods') ."as cg on cg.goods_id=g.goods_id and user_id='$_SESSION[user_id]' ";
      $isCollectSql_2 = 'cg.rec_id as collected, ';
  }

  /* 查询商品 */
  $sql = "SELECT g.goods_id, g.goods_name, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price ,g.goods_brief AS org_price, ".
                $isCollectSql_2.
  ...
  while ($row = $db->FetchRow($res))
  {
      ...
      $arr[$row['goods_id']]['goods_brief'] = $row['goods_brief'];
      $arr[$row['goods_id']]['collected'] = !empty($row['collected'])?1:0;
  }
...
//  $smarty->assign('promotion_info', get_promotion_info());
  $data = API_DATA("SIMPLEGOODS", $smarty->_var['goods_list']);
    ...
    if($category>0){
        $brands = getBrand($category,6);
    }else{
        $brands = array();
    }

GZ_Api::outPut($data, $pager);
//GZ_Api::outPut(array('brands'=>$brands,'goods'=>$data), $pager);
