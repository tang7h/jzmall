...
switch ($tmp[0]) {
	case 'checkOrder':
        ...
        //购物车里的商品选择后上传
        $goods_ids = _POST('goods_ids',array());
...
//		$smarty->assign('helps',            get_shop_help());       // 网店帮助
        ...
        /*------------------------------------------------------ */
           ...
           $user_id = $_SESSION['user_id'];
            ...
            $fileGoodsSql = '';
            if(!empty($goods_ids)){
                $fileGoodsSql = ' and c.goods_id '.db_create_in(array_values($goods_ids));
            }

		    /* 检查购物车中是否有商品 */
		    $sql = "SELECT COUNT(*) FROM " . $ecs->table('cart') .' as c '.
		        " WHERE (session_id = '" . SESS_ID . "' or user_id='{$user_id}') " .
		        "AND parent_id = 0 AND is_gift = 0 AND rec_type = '$flow_type'".$fileGoodsSql;
		    ...
		    /* 对商品信息赋值 */
		    $cart_goods = cart_goods($flow_type,$goods_ids,true); // 取得商品列表，计算合计
...
//                    if($payment['pay_code']!='alipay' &&  $payment['pay_code']!='weixin'){
//                        unset($payment_list[$key]);
//                    }
...
//		            }
	...
	case 'done':
	    ...
	    {
	        $cart_goods_stock = get_cart_goods($goods_ids);
	    ...
	    /* 订单中的商品 */
	    $cart_goods = cart_goods($flow_type,$goods_ids);
	   ...
	   /* 判断是不是实体商品 */
	    ...
	    /* 清空购物车 */
	    clear_cart($flow_type,$goods_ids);
