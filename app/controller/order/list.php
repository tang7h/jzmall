...
if (!in_array($type, array('await_pay', 'await_ship', 'shipped', 'finished', 'unconfirmed','await_comment','all','returned'))) {
	GZ_Api::outPut(101);
}

$record_count = 0;
if($type =='returned'){
    $record_count = $db->getOne("SELECT COUNT(*) FROM " .$ecs->table('order_return'). " as a WHERE user_id = '$user_id' ");
}else{
    $typeSql =  $type!='all'?GZ_order_query_sql($type,'a.'):'';
    $record_count = $db->getOne("SELECT COUNT(*) FROM " .$ecs->table('order_info'). " as a WHERE user_id = '$user_id' ".$typeSql);
}
...
//	$orders[$key]['order_time'] = formatTime($value['order_time']);//无需序列号订单时间
  ...
  $orders[$key]['order_info'] = array(
         ...
         'goods_number' => count($orders[$key]['goods_list']),
...
function GZ_get_user_orders($user_id, $num = 10, $start = 0, $type = 'await_pay')
{
    ...
    $typeSql = '';
    if($type == 'returned'){
        $returnSql = 'inner join '.$GLOBALS['ecs']->table('order_return').' as b on a.order_id=b.order_id ';
    }else{
        $typeSql = $type!='all'?GZ_order_query_sql($type,'a.'):'';
    }

    $sql = "SELECT a.order_id, a.order_sn, order_status, shipping_status, pay_status, a.add_time, " .
           "(goods_amount + shipping_fee + insure_fee + pay_fee + pack_fee + card_fee + tax - discount) AS total_fee ".
           " FROM " .$GLOBALS['ecs']->table('order_info') .
           "as a ".
           $returnSql.
           " WHERE a.user_id = '$user_id' " . $typeSql . " ORDER BY a.add_time DESC";
...
function GZ_order_goods($order_id)
{
    ...
    $sql = "SELECT o.*, " .
            "o.goods_price * o.goods_number AS subtotal,g.goods_thumb,g.original_img,g.goods_img " .$goodsInfoSql1.
