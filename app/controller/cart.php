...
switch ($tmp[0]) {
    ...
    case 'exchangeList':
        ...
        $pageSize = GZ_Api::$pagination['count'];
        ...
        $data = GZ_get_exchange_list($user_id,$page ,$pageSize,$record_count);
 ...
 * 获取积分商品列表
 */
function GZ_get_exchange_list($user_id=0,$start,$num,&$recordNum=0){

    $isCollectSql = '';
    $isCollectSql_2 = '';
    if ($user_id) {
        $isCollectSql = 'left join ' .$GLOBALS['ecs']->table('collect_goods') ."as cg on cg.goods_id=g.goods_id and user_id='$user_id' ";
        $isCollectSql_2 = 'cg.rec_id as goods_collected, ';
    }

    $sqlCount = 'select COUNT(*) from '
        . $GLOBALS['ecs']->table('exchange_goods').' as eg inner join '. $GLOBALS['ecs']->table('goods').' as g on eg.goods_id=g.goods_id '
        .' where g.is_delete = 0  and g.is_display=1 ';

    $sql= 'select g.seller_id,g.goods_id,g.goods_name,g.goods_thumb, g.goods_img, g.original_img, g.goods_brief, g.goods_type,'.$isCollectSql_2.' eg.* from '
        . $GLOBALS['ecs']->table('exchange_goods').' as eg inner join '. $GLOBALS['ecs']->table('goods').' as g on eg.goods_id=g.goods_id '
        . $isCollectSql
        .' where g.is_delete = 0  and g.is_display=1 '
        .'';

    $recordNum = $GLOBALS['db']->getOne($sqlCount)+0;
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $temp['goods_thumb']           = $row['goods_thumb'];
        $temp['goods_brief']           = $row['goods_brief'];
        $temp['goods_type']           = $row['goods_type'];
        $temp['goods_img']           = $row['goods_img'];
        $temp['goods_point']          = $row['exchange_integral'];
        $temp['seller_id']        = $row['seller_id'];
        $temp['goods_collected']  = empty($row['goods_collected'])?0:$row['goods_collected'];
