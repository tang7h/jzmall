...
//	$sql="update ".$ecs->table('seller_shopheader')." set content='$shop_header' where seller_id='".$_SESSION['seller_id']."' and seller_theme='".$seller_theme."'";
    $sql="update ".$ecs->table('seller_shopheader')." set content='$shop_header' where seller_id='".$_SESSION['seller_id']."' ";
	$db->query($sql);
...
elseif ($_REQUEST['act'] == 'first')
{
    ...
    $smarty->display('seller_shop_config.htm');
}
...
elseif ($_REQUEST['act'] == 'second')
{
	...
	$map_api_x=$_POST['map_api_x'];
	$map_api_y=$_POST['map_api_y'];
	$shop_info=array(
