...
//        $this->assign('total', $cart_goods ['total']);
        ...
        if ($cart_goods['goods_list']) {
            ...
            $linked_goods = model('Goods')->get_linked_goods($cart_goods);
...
//        $this->assign('market_price_desc', sprintf(L('than_market_price'), $cart_goods ['total'] ['market_price'], $cart_goods ['total'] ['saving'], $cart_goods ['total'] ['save_rate']));
    ...
    public function checkout() {
        ...
        $seller_cart = array();
        foreach ($cart_goods as $key => $val) {
            $seller_cart[$key]['seller'] = M()->getRow("select id,shop_name from " . $this->model->pre . "seller_shopinfo where seller_id='$key'");
            $seller_cart[$key]['shippings']=model('Order')->seller_available_shipping_list($region,$key);
            $seller_cart[$key]['cart_weight_price'] =model('Order')->seller_cart_weight_price($flow_type,$key);
            $seller_cart[$key]['goods_list'] = $val;
        ...
        foreach($seller_cart as $key=>$val)
        {
            ...
            $shipping_count = M()->table('cart')->field('count(*)')->where(" `session_id` = '" . SESS_ID. "' AND `extension_code` != 'package_buy' AND `is_shipping` = 0 and seller_id='$key'")->getOne();
        ...
        $this->assign('seller_cart', $seller_cart);
        $this->assign('count', count($seller_cart));
        /* 对是否允许修改购物车赋值 */
    	...
    	foreach($cart_goods as $key=>$val)
    	{
    		$seller_total[$key]['total']=model('Order')->seller_order_fee($order, $val, $consignee);
    	}
	   ...
	   $cart_goods=model('Order')->cart_goods($flow_type);
    	...
    	{
    		$total = model('Order')->seller_order_fee($order, $cart_goods, $consignee);
    	...
    	{
    		$this->assign('total', $_SESSION['total']);	
    	}
        ...
        /*   使用商城券   */
        //获得会员所拥有的商城券
        $this->assign('wheat_coin_v',C('wheat_coin'));//系统设置的商城券实际人民币价值
        $user_wheat_coin = M()->table('users')->field('wheat_coin')->where(" user_id='".$_SESSION['user_id']."'")->getOne();
        $this->assign('user_wheat_coin', $user_wheat_coin);
        /* 如果使用余额，取得用户余额 */
    ...
    public function select_shipping() {
        ...
        if (empty($cart_goods) || !model('Order')->check_consignee_info($consignee, $flow_type)) {
            ...
            $seller_total = session('seller_total');
            $seller_total[$seller_id]['shipping_id'] = $order['shipping_id'];
            session('seller_total',$seller_total);
            ...
            $_SESSION['seller_total'][$seller_id]=model('Users')->order_fee($order, $cart_sup_goods[$seller_id], $consignee);
            ...
            $total['sub_coin'] = session('sub_coin') ? session('sub_coin') : 0;
            $order_amount=$total['amount']+$total['pay_fee']-$total['sub_coin'];
            $total['amount_formated']=price_format($order_amount, false);
            $total['all_amount']=$order_amount;
            $total['shipping_insure_formated']=price_format($total['shipping_insure'], false);
            $total['shipping_totelfee_formated']=price_format($_SESSION['total']['shipping_fee'], false);
            $this->assign('total',$total);
            session('total',$total);
            /* 取得可以得到的积分和红包 */
            $this->assign('total_integral', model('Order')->cart_amount(false, $flow_type) - $total ['bonus'] - $total ['integral_money']);
    ...
    public function done() {
        ...
        /* 订单中的商品 */
        // 商家入驻
        $cart_seller_goods = model('Users')->cart_sup_goods($flow_type);
        ...
        foreach ($cart_seller_goods as $seller_id => $cart_goods) {
            ...
            if(isset($is_real_good)){
                if(!M()->table('shipping')->field('shipping_id')->where( " shipping_id=".$seller_order['shipping_id'] ." AND enabled =1")->getOne()){
                    show_message(L('flow_no_shipping'));
                }
            }
            ...
            $order['tax']          = $total['tax'];
            $order['wheat_coin']=$seller_order['sub_coin']?intval($seller_order['sub_coin'])/C('wheat_coin'):0;
            ...
            do {
                ...
                if (!empty($fields)) {
                    $sql = 'INSERT INTO ' . $this->model->pre . 'order_info (' . implode(', ', $fields) . ') VALUES (' . implode(', ', $values) . ')';
                }
                $this->model->query($sql);
            ...
            /* 插入订单商品 */
            $sql = "INSERT INTO " . $this->model->pre . "order_goods( " . "order_id, goods_id, goods_name, goods_sn, product_id, goods_number, market_price, " . "goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id) " . " SELECT '$new_order_id', goods_id, goods_name, goods_sn, product_id, goods_number, market_price, " . "goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id" . " FROM " . $this->model->pre . "cart WHERE session_id = '" . SESS_ID . "' AND rec_type = '$flow_type' and seller_id='$seller_id'";
            $this->model->query($sql);
            /* 修改拍卖活动状态 */
            ...
            if($order['wheat_coin']>0)
            {
                ...
                $u_coin_sql="update ".M()->pre . 'users '." set wheat_coin=(wheat_coin-".$order['wheat_coin'].") where user_id='".session('user_id')."'";
                M()->query($u_coin_sql);
            }
            /* 如果使用库存，且下订单时减库存，则减少库存 */
            if (C('use_storage') == '1' && C('stock_dec_time') == SDT_PLACE) {
                model('Order')->change_order_goods_storage($new_order_id, true, SDT_PLACE);
            }
            /* 如果订单金额为0 处理虚拟卡 */
            if ($order ['order_amount'] <= 0) {
                ...
                if ($virtual_goods and $flow_type != CART_GROUP_BUY_GOODS) {
                    ...
                    if (model('OrderBase')->virtual_goods_ship($virtual_goods, $msg, $order ['order_sn'], true)) {
                        ...
                        $count = $this->model->table('order_goods')->field('COUNT(*)')->where("order_id = '$new_order_id' " . " AND is_real = 1")->getOne();
                        if ($count <= 0) {
                            /* 修改订单状态 */
                            model('Users')->update_order($new_order_id, array(
                            ...
                            if ($order ['user_id'] > 0) {
                                ...
                                /* 发放红包 */
                                model('Order')->send_order_bonus($new_order_id);
                            }
                        }
                    }
                }
            }
            $TotalArray[$seller_id] = $total;
            $OrderArray[$seller_id] = $order;
            $OrderArray[$seller_id]['amount_formated'] = $total['amount_formated'];
        }
        $this->assign('OrderCount', count($OrderArray));
        $this->assign('TotalArray', $TotalArray);
        ...
        $this->assign('order_count',count($orders));// 商家入驻
    ...
    public function select_payment() {
		...
		foreach($cart_sup_goods as $key=>$val)
		{
			...
			$_SESSION['seller_total'][$key]=model('Users')->order_fee($order, $val, $consignee);
    ...
    public function select_pack() {
        ...
        if (empty($cart_goods) || !model('Order')->check_consignee_info($consignee, $flow_type)) {
           ...
           $order_counts = count($cart_goods);
            ...
            $total['sub_coin'] = session('sub_coin');
            $order_amount=$total['amount']+$total['pay_fee']-$total['sub_coin'];
            $total['amount_formated']=price_format($order_amount, false);
            $total['all_amount']=$order_amount;
            $total['shipping_insure_formated']=price_format($total['shipping_insure'], false);
    ...
    public function change_bonus() {
        ...
        $total = model('Order')->seller_order_fee($order, $cart_goods, $consignee,count($cart_sup_goods));
            $this->assign('total', $total);
    ...
    else
    {
		...
		{
			//使用的商城券不可以超过所拥有的商城券
			if((($_SESSION['total']['sub_coin']+$_SESSION['seller_total'][$key]['sub_coin'])/C('wheat_coin'))>$user_wheat_coin)
			{
