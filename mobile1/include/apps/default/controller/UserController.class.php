...
class UserController extends CommonController {
     ...
     * 异步加载评论列表
     */
    public function async_comment_list() {
        if (IS_AJAX) {
            $start = $_POST['last'];
            $limit = $_POST['amount'];
            $comment_list = model('ClipsBase')->get_comment_list($this->user_id, 5, 0);
     ...
     * 商城券管理
     */
    public function wheat_coin(){
       $wheat_coin = M()->table('users')->field('wheat_coin')->where("user_id = ".$this->user_id)->getOne();
       $this->assign('wheat_coin',$wheat_coin);
       
       $account_type = 'wheat_coin';
       
       $where = " user_id = '$this->user_id' ";
       $where .= " AND $account_type <> 0 ";
       
       $filter['page'] = '{page}';
       $offset = $this->pageLimit(url('wheat_coin', $filter), 5);
       $offset_page = explode(',', $offset);
       $count = $this->model->table('account_log')->where($where)->count();
       
       $sql = "SELECT * FROM " . M()->pre . 'account_log where ' . $where . " ORDER BY log_id DESC limit $offset_page[0], $offset_page[1]";
       
       $account_list = M()->query($sql);
       
       foreach ($account_list as $key=>$val){
           $account_list[$key]['change_time'] = local_date(C('time_format'), $val['change_time']);
       }
       $this->assign('account_list', $account_list);
       $this->assign('page', $this->pageShow($count));
       $this->assign('title','商城券管理');
       $this->display('user_wheat_coin.dwt');
    }
    
    /**
     * 商城券赠送
     */
    public function wheat_coin_gift(){
        if (IS_POST){
            // 判断输入信息是否正确
            $user_name_sk = I('post.user_name_sk');
            $wheat_coin_zz = I('post.wheat_coin');
            if (empty($user_name_sk)){
                show_message('请输入收款人帐号');
            }
            if ($wheat_coin_zz <= 0){
                show_message('赠送商城券必须大于0');
            }
            // 判断转账是否满足条件
            $wheat_coin = M()->table('users')->field('wheat_coin')->where("user_id = ".$this->user_id)->getOne();
            if ($wheat_coin_zz > $wheat_coin){
                show_message('帐号商城券不足以支付');
            }
            $user_id_sk = M()->table('users')->field('user_id')->where("user_name = '". $user_name_sk ."'")->getOne();
            if (!$user_id_sk){
                show_message('收款人帐号错误');
            }
            // 转账
            $data['wheat_coin'] = ($wheat_coin-$wheat_coin_zz);
            $this->model->table('users')->data($data)->where("user_id = ".$this->user_id)->update();
            unset($data);
            $wheat_coin_sk = M()->table('users')->field('wheat_coin')->where("user_name = '". $user_name_sk ."'")->getOne();
            $data['wheat_coin'] = ($wheat_coin_sk+$wheat_coin_zz);
            M()->table('users')->data($data)->where("user_name = '". $user_name_sk ."'")->update();
            $change_desc = session('user_name')." 赠送给 ".$user_name_sk .' '. $wheat_coin_zz . "个商城券";
            // 插入资金管理日志
            $account_log = array(
                'user_id'       => $this->user_id,
                'change_time'   => gmtime(),
                'change_desc'   => $change_desc,
                'wheat_coin'   => '-'.$wheat_coin_zz,
                'change_type'   => ACT_OTHER
            );
            M()->table('account_log')->data($account_log)->insert();
            ...
            M()->table('account_log')->data($account_log)->insert();
            show_message('赠送成功','返回商城券管理',url('user/wheat_coin'));
        }else{
            $wheat_coin = M()->table('users')->field('wheat_coin')->where("user_id = ".$this->user_id)->getOne();
            $this->assign('wheat_coin',$wheat_coin);
            $this->assign('title','赠送商城券');
            $this->display('user_wheat_coin_gift.dwt');
        }
    }
