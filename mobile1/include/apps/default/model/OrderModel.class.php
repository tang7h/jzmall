...
class OrderModel extends BaseModel {
    ...
    function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0) {
        ...
        /* 取得商品信息 */
        $sql = "SELECT g.goods_name, g.goods_sn, g.is_on_sale, g.is_real,g.seller_id,  " .
    ...
    function flow_order_info() {
        ...
        if (!isset($order['sub_coin'])) {
            $order['sub_coin'] = 0;  // 初始化有麦币
        }
        /* 扩展信息 */
...
//    }
    ...
    function seller_available_shipping_list($region_id_list,$seller_id=0)
    {
        $sql = 'SELECT s.shipping_id, s.shipping_code, s.shipping_name, ' .
            ...
            'FROM ' . M()->pre . 'shipping' . ' AS s, ' .
            M()->pre . 'shipping_area' . ' AS a, ' .
            M()->pre . 'area_region' . ' AS r ' .
    ...
    function seller_cart_weight_price($type = CART_GENERAL_GOODS,$seller_id=0)
    {
        ...
        /* 计算超值礼包内商品的相关配送参数 */
        $sql = 'SELECT goods_id, goods_number, goods_price FROM ' . M()->pre . 'cart' . " WHERE extension_code = 'package_buy' AND session_id = '" . SESS_ID . "' and seller_id='".$seller_id."'";
        $row = M()->query($sql);
        ...
        if ($row)
        {
            ...
            foreach ($row as $val)
            {
                // 如果商品全为免运费商品，设置一个标识变量
                $sql = 'SELECT count(*) as count FROM ' .
                    M()->pre . 'package_goods' . ' AS pg, ' .
                    M()->pre . 'goods' . ' AS g ' .
                    "WHERE g.goods_id = pg.goods_id AND g.is_shipping = 0 AND pg.package_id = '"  . $val['goods_id'] . "'";
                $shipping_count = M()->getRow($sql);
                $shipping_count = $shipping_count['count'] ? $shipping_count['count'] : 0;
                if ($shipping_count > 0)
                {
                    // 循环计算每个超值礼包商品的重量和数量，注意一个礼包中可能包换若干个同一商品
                    $sql = 'SELECT SUM(g.goods_weight * pg.goods_number) AS weight, ' .
        ...
        $sql    = 'SELECT SUM(g.goods_weight * c.goods_number) AS weight, ' .
            'SUM(c.goods_price * c.goods_number) AS amount, ' .
            'SUM(c.goods_number) AS number '.
            'FROM ' . M()->pre . 'cart' . ' AS c '.
            'LEFT JOIN ' . M()->pre . 'goods' . ' AS g ON g.goods_id = c.goods_id '.
            "WHERE c.session_id = '" . SESS_ID . "' " .
    ...
    function seller_order_fee($order, $goods, $consignee,$order_counts=1)
    {
        ...
        if($order['extension_code'] != 'group_buy')
        {
            $discount = $this->compute_discount();
        ...
        if (!empty($order['need_inv']) && $order['inv_type'] != '')
        {
            /* 查税率 */
            $rate = 0;
            foreach (C('invoice_type.type') as $key => $type)
            {
                if ($type == $order['inv_type'])
                {
                    $rate = floatval(C('invoice_type.rate.'.$key)) / 100;
                    break;
                }
            }
            if ($rate > 0)
        ...
        if ($order['shipping_id'] > 0 && $total['real_goods_count'] > 0)
        {
            ...
            if (!empty($shipping_info))
            {
                if ($order['extension_code'] == 'group_buy')
                {
                    $weight_price = $this->cart_weight_price(CART_GROUP_BUY_GOODS);
                }
                else
                {
                ...
                $shipping_count = M()->table('cart')->field('count(*)')->where(" `session_id` = '" . SESS_ID. "' AND `extension_code` != 'package_buy' AND `is_shipping` = 0")->getOne();
        ...
        $total['shipping_totelfee_formated']    = price_format($total['shipping_fee']*$order_counts, false);
        ...
        $total['will_get_bonus']        = $order['extension_code'] == 'exchange_goods' ? 0 : price_format($this->get_total_bonus(), false);
        ...
        {
            $sql = 'SELECT SUM(eg.exchange_integral) as sum '.
                'FROM ' . M()->pre . 'cart' . ' AS c,' . M()->pre . 'exchange_goods' . 'AS eg '.
            ...
            $exchange_integral = M()->getRow($sql);
            $exchange_integral = $exchange_integral['sum'] ? $exchange_integral['sum'] : 0;
