...
class UsersModel extends BaseModel {
    ...
    function get_user_orders($user_id, $pay = 1, $num = 10, $start = 0) {
        ...
        $sql = "SELECT order_id, order_sn, shipping_id, order_status, shipping_status, pay_status, add_time,seller_id, " .
     ...
     * @return  array
     */
    function order_fee($order, $goods, $consignee, $order_counts = 0) {
...
//         $total['shipping_totelfee_formated'] = price_format($total['shipping_fee'] * $order_counts, false);
        ...
        /* 有麦币 */
        $order['sub_coin'] = $order['sub_coin'] > 0 ? $order['sub_coin'] : 0;
        if ($total['amount'] > 0) {
            if (isset($order['sub_coin']) && $order['sub_coin'] > $total['amount']) {
                $order['sub_coin'] = $total['amount'];
                $total['amount'] = 0;
            } else {
                $total['amount'] -= floatval($order['sub_coin']);
            }
        } else {
            $order['sub_coin'] = 0;
            $total['amount'] = 0;
        }
        $total['sub_coin'] = $order['sub_coin'];
        $total['sub_coin_formated'] = price_format($order['sub_coin'], false);
