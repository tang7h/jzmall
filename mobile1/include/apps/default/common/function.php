...
function get_location($Longitude,$Latitude){
    ...
    // 将ip转换为百度地图经纬度
    $url = "http://api.map.baidu.com/geoconv/v1/?coords=$Longitude,$Latitude&from=1&to=5&ak=qkEAvQuWyUjmsqEc5Iyuda9R";
    $address  = Http::doGet($url);
    $address  = json_decode($address,true);
    $x = $address['result']['0']['x'];
    $y = $address['result']['0']['y'];

    $url = "http://api.map.baidu.com/geocoder/v2/?ak=qkEAvQuWyUjmsqEc5Iyuda9R&location=$y,$x&output=json";
    $ip  = Http::doGet($url);
    $ip  = json_decode($ip,true);
    
    if($ip['status'] > 0){
    }else {
        $address['city'] = $ip['result']['addressComponent']['city'];
        $address['district'] = $ip['result']['addressComponent']['district'];
        $address['province'] = $ip['result']['addressComponent']['province'];
        default_location($address);
       
    }
}
    
    function default_location($address){
        ...
        {
            $ip_where1=!empty($ip_where1)?" and province like'%".$address['province']."%'":" province like'%".$address['province']."%'";
        }
        if(!empty($address['city']))
        {
            $ip_where2 = !empty($ip_where1)?" and city like'%".$address['city']."%'":" city like'%".$address['city']."%'";
            $ip_where2 = $ip_where1.$ip_where2;
        }
        if(!empty($address['district']))
        {
            $ip_where3 =!empty($ip_where2)?" and district like'%".$address['district']."%'":" district like'%".$address['district']."%'";
            $ip_where3 = $ip_where2.$ip_where3;
        }
        if(!empty($ip_where3))
        {
            $SellerIdArray3 =M()->table('user_seller')->field('id')->where($ip_where3)->getCol();
            if ($SellerIdArray3){
                foreach ($SellerIdArray3 as $key=>$val){
                    $SellerIdArray[] = $val['id'];
                }
            }
        }
        if (!empty($ip_where2)){
            $SellerIdArray2 = M()->table('user_seller')->field('id')->where($ip_where2 ." and " . db_create_in($SellerIdArray,' id NOT '))->getCol();
            if ($SellerIdArray2){
                foreach ($SellerIdArray2 as $key=>$val){
                    $SellerIdArray[] = $val['id'];
                } 
            }
        ...
        if (!empty($ip_where1)){
            $SellerIdArray1 = M()->table('user_seller')->field('id')->where($ip_where3 . " and " . db_create_in($SellerIdArray,' id NOT '))->getCol();
            if ($SellerIdArray1){
                foreach ($SellerIdArray1 as $key=>$val){
                    $SellerIdArray[] = $val['id'];
                }
            }
        }
...
//         echo $ip_where3,'<br>';
//         echo $ip_where2,'<br>';
//         echo $ip_where1,'<br>';exit;
        session('SellerIdArray',$SellerIdArray);
    }
