...
'account_deposit', 'account_log', 'account_detail', 'act_account', 'pay', 'default', 'bonus', 'group_buy', 'group_buy_detail', 'affiliate', 'comment_list','validate_email','track_packages', 'transform_points','qpassword_name', 'get_passwd_question', 'check_answer','baitiao','repay_bt','return_list' , 'apply_return' , 'apply_info' , 'submit_return', 'goods_order' , 'return_detail' , 'edit_express' , 'return_shipping');
...
elseif($action == 'comm_order_goods')
{
	...
	$ip = real_ip();
	//wang 商家入驻 获取商品商家id
	$seller_id=$db->getOne("select seller_id from ".$ecs->table('goods')." where goods_id='$goods_id'");
	...
	if($db->query($sql))
	{
		if($status)
		{
			...
			show_message('评论成功', $_LANG['back_page_up'], "user.php?act=single_list", 'success');
		}
		else
		{
			show_message('评论成功，请等待管理员审核', $_LANG['back_page_up'], "user.php?act=single_list", 'success');
		}
...
elseif( $action == 'submit_return'){
    ...
    $sql = " SELECT country,province,city ,district FROM ".$GLOBALS['ecs']->table('order_info')." WHERE order_id =" .$order_id;
    ...
    $order_return = array(
        ...
        'back'       => empty( $_REQUEST['check_return'])?'' : intval( $_REQUEST['return_num'])  ,
        'user_id'    =>  $_SESSION['user_id'], 
        'exchange'   => empty( $_REQUEST['attr_num'])? 0 : count( $_REQUEST['attr_num']), 
        'goods_attr' => empty( $_REQUEST['goods_attr'])?0 : $_REQUEST['goods_attr'],   //换出商品属性
        'attr_val'   => empty( $_REQUEST['array_attr']) ? '' : get_return_attr($_REQUEST['array_attr'] ),     // 换回商品属性
        'attr_num'   => empty( $_REQUEST['attr_num']) ? 0  : $_REQUEST['attr_num'],
        'remark'     => empty( $_REQUEST['remark'])  ? '' : $_REQUEST['remark'] ,
        'country'    => $res['country'],
        'province'   => $res['province'],
        'city'       => $res['city'],
        'district'   => $res['district'],
        'cause_id'   => empty( $_REQUEST['last_option']) ? 0 : $_REQUEST['last_option'],
        'apply_time'   => gmtime(),
        'actual_return' => '',
        'address'    => empty( $_REQUEST['address']) ? '': $_REQUEST['address'],
        'zipcode'    => empty( $_REQUEST['code']) ?'' : $_REQUEST['code'],
        'addressee'  => empty( $_REQUEST['hid1'] )? '' : $_REQUEST['hid1'],
        'return_status' => '0'
    );
   
    $order_return['should_return'] = get_return_refound($order_return['order_id'] , $order_return['rec_id'] ,$order_return['back']);
    $return_goods['refound'] = empty( $_REQUEST['goods_price'])? '' : $_REQUEST['goods_price']; 

    if ($query = $db->autoExecute($ecs->table('order_return'), $order_return, 'INSERT', '', 'SILENT'))
    {
        $ret_id = mysql_insert_id();
           /* 记录log */
        return_action( $ret_id,'申请退款（由用户寄回）', '', $order_return['remark'],'买家');  
        
        $return_goods['rec_id'] = $order_return['rec_id'];
        $return_goods['goods_id'] = $order_return['goods_id'];
        $return_goods['goods_name'] = empty($_REQUEST['goods_name'])?'':$_REQUEST['goods_name'];
        $return_goods['goods_sn'] = empty( $_REQUEST['goods_sn']) ? '' : $_REQUEST['goods_sn'];
        $return_goods['is_real']  =   empty($_REQUEST['is_real']) ? 0 : 1 ; 
        $return_goods['goods_attr'] = empty( $_REQUEST['goods_attr'])? '' : $_REQUEST['goods_attr']; 
        $return_goods['refound'] = empty( $_REQUEST['goods_price'])? '' : $_REQUEST['goods_price']; 
        //添加到退换货商品表中
        if( $order_return['back']){
            //添加退货
            
            $return_goods['return_type'] = 1; //退货
            $return_goods['back_num'] = empty($order_return['back'])?0: $order_return['back'];
            $return_goods['out_attr'] = 0 ;
            $return_goods['out_num'] = 0;
           
            $query = $db->autoExecute($ecs->table('return_goods'), $return_goods, 'INSERT', '', 'SILENT');
            
            
        }
        if( $order_return['exchange'] ){
            //添加换货
         
                $attr = unserialize( $order_return['attr_val'] );
...
elseif ( $action == 'ajax_select_cause'){
    ...
    if (isset($c_id) && isset($rec_id)) {
        ...
        $select = '<select name="last_option">';
