...
if ($_REQUEST['act'] == 'list')
{
	...
	//ecmoban模板堂 --zhuo
	$smarty->assign('action_link2',  array('text' => $_LANG['12_users_export'], 'href'=>'users.php?act=export'));
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    u.office_phone, u.home_phone, u.mobile_phone,u.wheat_coin ".
        " FROM " .$ecs->table('users'). " u LEFT JOIN " . $ecs->table('users') . " u2 ON u.parent_id = u2.user_id WHERE u.user_id='$_GET[id]'";
    ...
    if ($row)
    {
		...
		$user['wheat_coin']   = $row['wheat_coin'];
    }
    else
    {
          $link[] = array('text' => $_LANG['go_back'], 'href'=>'users.php?act=list');
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    $rank = empty($_POST['user_rank']) ? 0 : intval($_POST['user_rank']);
    $credit_line = empty($_POST['credit_line']) ? 0 : floatval($_POST['credit_line']);
	$wheat_coin = empty($_POST['wheat_coin']) ? 0 : intval($_POST['wheat_coin']);
...
function user_list()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT user_id, user_name, email, is_validated, user_money, frozen_money, rank_points, pay_points, reg_time,wheat_coin ".
...
function export_csv() {
	...
	echo user_date(get_export_data());
}
function user_date($result) {
    if(empty($result)) {
        return i("没有符合您要求的数据！^_^");
    }
    $data = i('编号,会员名称,邮件地址,是否已验证,可用资金,冻结资金,等级积分,消费积分,注册日期'."\n");
    $count = count($result);
    for($i = 0 ; $i < $count ;  $i++) {
        $data .= i($result[$i]['user_id']).','.i($result[$i]['user_name']).','.i($result[$i]['email']).','.i($result[$i]['is_validated']).','.i($result[$i]['user_money']).','.i($result[$i]['frozen_money']).','.i($result[$i]['rank_points']).','.i($result[$i]['pay_points']).','.i($result[$i]['reg_time'])."\n";
    }
    return $data;
}
function get_export_data() {
    $sql = "select user_id, user_name, email, is_validated, user_money, frozen_money, rank_points, pay_points, reg_time from " . $GLOBALS['ecs']->table('users') . " where user_id > 0";
	$res = $GLOBALS['db']->getAll($sql);
    foreach($res as $key=>$row){
        $arr[$key]['user_id'] = $row['user_id'];
        $arr[$key]['user_name'] = $row['user_name'];
		$arr[$key]['email'] = $row['email'];
        $arr[$key]['is_validated'] = $row['is_validated'];
		$arr[$key]['user_money'] = $row['user_money'];
        $arr[$key]['frozen_money'] = $row['frozen_money'];
		$arr[$key]['rank_points'] = $row['rank_points'];
        $arr[$key]['pay_points'] = $row['pay_points'];
		$arr[$key]['reg_time'] = local_date("Y-m-d", $row['reg_time']);
