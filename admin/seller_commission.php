...
elseif ($_REQUEST['act'] == 'info')
{
	...
	{
		//统计总营业额
		$sql="select sum(goods_amount) as number,order_id from " . $GLOBALS['ecs']->table('order_info') ." where order_status=5 and seller_id='".$comm_info['seller_id']."' and add_time>=".$comm_info['prev_date']." and add_time<=".gmtime();
...
function comm_list($is_pager=true)
{
    ...
    foreach ($row AS $key => $value)
    {
		...
		{
			//统计总营业额
			$sql="select sum(goods_amount) as number from " . $GLOBALS['ecs']->table('order_info') ." where order_status=5 and seller_id='".$value['seller_id']."' and add_time>=".$value['prev_date']." and add_time<=".gmtime() ;
			$total_turnover=$GLOBALS['db']->getOne($sql);
			//获取商家分成百分比
