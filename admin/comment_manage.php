...
if ($_REQUEST['act']=='reply')
{
	...
	$comment_info['advantages']  = str_replace('\r\n', '<br />', htmlspecialchars($comment_info['advantages']));
    $comment_info['advantages']  = nl2br(str_replace('\n', '<br />', $comment_info['advantages']));
	$comment_info['defect']  = str_replace('\r\n', '<br />', htmlspecialchars($comment_info['defect']));
    $comment_info['defect']  = nl2br(str_replace('\n', '<br />', $comment_info['defect']));
	//ecmoban模板堂 --zhuo end
...
if ($_REQUEST['act'] == 'check')
{
    if ($_REQUEST['check'] == 'allow')
    {
		...
		get_update_user_commets($_REQUEST['id'],1); //ecmoban模板堂 --zhuo
    ...
    if (isset($_POST['checkboxes']))
    {
        switch ($action)
        {
           ...
           case 'allow' :
			   ...
			   get_update_user_commets(implode(",",$_POST['checkboxes']),1,$_POST['checkboxes']); //ecmoban模板堂 --zhuo
               break;
