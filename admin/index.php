...
elseif ($_REQUEST['act'] == 'top')
{
	...
	$smarty->assign('orderTime_goods', get_orderTime_goods());
	$smarty->assign('rand_order', $_CFG['rand_order']);
...
elseif ($_REQUEST['act'] == 'order_dayTime')
{
	...
	$sql = "select randOrder from " .$ecs->table('goods'). " where goods_id = '$goods_id'";
	$randOrder = $db->getOne($sql);
	
	$sql="update ". $ecs->table('goods')." set order_dayTime = '$order_dayTime' WHERE goods_id='$goods_id'";
	$db->query($sql);
	
	if($randOrder){
		$orderNum = rand(1,$orderNum); //随机获取
		
		$file_old = ROOT_PATH . 'virtual_goods/' . $goods_id . '.txt'; 
		
		if(file_exists($file_old)){
			$old_virtual_gmunber = file_get_contents($file_old);  //获取文本虚拟商品销量
			$old_orderNum = $old_virtual_gmunber + $orderNum;
		}else{
	...
	$file_new = ROOT_PATH . 'virtual_goods/' . $goods_id . '.txt';
	file_put_contents($file_new,$old_orderNum); //存储在文本中
	
	/*$sql="update ". $ecs->table('goods')." set order_gmunber = order_gmunber + '$old_orderNum' WHERE goods_id='$goods_id'";
	$db->query($sql);*/  //不可取（保留此功能）
...
elseif ($_REQUEST['act'] == 'license')
{
    ...
    if (isset($is_ajax) && $is_ajax)
    {
        ...
        $confirmkey=licenseEncode($sess->get_session_id() , $license['request']['info']['certificate_id']);
                        $bsrt['auth_str']='http://service.shopex.cn/info.php?certi_id='.$license['request']['info']['certificate_id'].'&version=ecshop&sess_id='.$sess->get_session_id().'&confirmkey='.$confirmkey.'&_key_=do';
        $bsrt=array();
        $bsrt['auth']=1;

        $bsrt['auth_type']=$_LANG['authorize'];

        $fsrt=array();
        $fsrt['auth']=0;
        $fsrt['auth_str']='license.php?act=list_edit';
        $fsrt['auth_type']=$_LANG['nauthorize'];

        switch ($license['flag'])
        {
            case 'login_succ':
                if ($license['request']['res']=='succ' &&  $license['request']['info']['service']['ecshop_b2c']['cert_auth']['service_time']['etime']>0)
                {
                  
                        $confirmkey=licenseEncode($sess->get_session_id() , $license['request']['info']['certificate_id']);
                        $bsrt['auth_str']='http://service.shopex.cn/info.php?certi_id='.$license['request']['info']['certificate_id'].'&version=ecshop&sess_id='.$sess->get_session_id().'&confirmkey='.$confirmkey.'&_key_=do';
                    make_json_result($bsrt);
                }
                else
                {
                    make_json_result($fsrt);
                }
            break;
...
//ecmoban模板堂 --zhuo start 2013-12-3
function get_orderTime_goods(){
	$sql = "select goods_id,orderDay,orderHours,orderMinute,orderNum from " .$GLOBALS['ecs']->table('goods'). " where 1";
	$res = $GLOBALS['db']->getAll($sql);
	
	$arr = array();
	foreach($res as $row){
		$arr[$row['goods_id']]['goods_id'] 		= $row['goods_id'];
		$arr[$row['goods_id']]['orderDay'] 		= $row['orderDay'];
		$arr[$row['goods_id']]['orderHours'] 	= $row['orderHours'];
		$arr[$row['goods_id']]['orderMinute'] 	= $row['orderMinute'];
		$arr[$row['goods_id']]['orderNum'] 		= $row['orderNum'];
		$arr[$row['goods_id']]['orderDay_all'] 		= ($row['orderDay'] * 24 * 3600) + ($row['orderHours'] * 3600) +  ($row['orderMinute'] * 60);
	}
