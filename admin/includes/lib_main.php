 ...
 * @return  void
 */
function notice_log($goods_name,$email,$send_ok,$send_type)
{
    $sql = 'INSERT INTO ' . $GLOBALS['ecs']->table('notice_log') . ' (goods_name, email, send_ok, send_time, send_type) ' .
            " VALUES ('$goods_name', '$email', '$send_ok', '" . gmtime() . "', '$send_type')";
...
{
    $sql = 'SELECT cat_id, cat_name FROM ' . $GLOBALS['ecs']->table('goods_type') . " WHERE enabled = 1 and seller_id='$_SESSION[seller_id]'";
    $res = $GLOBALS['db']->query($sql);
...
/**
 * 检查退换货原因是否有存在 by Leah
 * @param type $cause_name
 * @return type
 */
function cause_exists( $cause_name){
