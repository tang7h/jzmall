...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy')
{
    ...
    if ($is_add)
    {
        ...
        $goods = array(
			...
			'group_integral' => 0,      //ecmoban模板堂 --zhuo
			'group_integral_num' => 0,	//ecmoban模板堂 --zhuo
			'virtual_num' => 0,	//ecmoban模板堂 --zhuo
    ...
    else
    {
		...
		//ecmoban模板堂 --zhuo start 2013-12-3
		if (!empty($goods['order_dayTime']))
        {
            $goods['order_dayTime']    = local_date('Y-m-d H:i:s', $goods['order_dayTime']);
			$goods['order_dayTime'] = str_replace('-','/',$goods['order_dayTime']);
        }
		    
		$goods['inputTime'] = ($goods['orderDay'] * 24 * 3600) + ($goods['orderHours'] * 3600) +  ($goods['orderMinute'] * 60);
		$goods['rand_order'] = $_CFG['rand_order'];
		//ecmoban模板堂 --zhuo end 2013-12-3
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
	...
	$wheat_coin = !empty($_POST['wheat_coin']) ? $_POST['wheat_coin'] : 0;
	...
	//by guan start
    $pin = new pin();
    $pinyin = $pin->Pinyin($_POST[goods_name],'UTF8');
    //by guan end
	
	$group_integral = !empty($_POST['group_integral'])? $_POST['group_integral'] : 0; //ecmoban模板堂 --zhuo
	$group_integral_num = !empty($_POST['group_integral_num'])? $_POST['group_integral_num'] : 0; //ecmoban模板堂 --zhuo
	$virtual_num = !empty($_POST['virtual_num'])? $_POST['virtual_num'] : 0; //ecmoban模板堂 --zhuo
	
	$orderDay = !empty($_POST['orderDay'])? $_POST['orderDay'] : 0; //ecmoban模板堂 --zhuo 2013-12-3
	$orderHours = !empty($_POST['orderHours'])? $_POST['orderHours'] : 0; //ecmoban模板堂 --zhuo 2013-12-3
	$orderMinute = !empty($_POST['orderMinute'])? $_POST['orderMinute'] : 0; //ecmoban模板堂 --zhuo 2013-12-3
	$orderNum = !empty($_POST['orderNum'])? $_POST['orderNum'] : 0; //ecmoban模板堂 --zhuo 2013-12-3
	$randOrder = !empty($_POST['randOrder'])? $_POST['randOrder'] : 0; //ecmoban模板堂 --zhuo 2013-12-3

	//ecmoban模板堂 --zhuo start 2013-12-3
	if($orderDay || $orderHours || $orderMinute || $orderNum){
		$order_dayTime = gmtime() + ($orderDay * 24 * 3600) + ($orderHours * 3600) +  ($orderMinute * 60); 
	}
	//ecmoban模板堂 --zhuo end 2013-12-3
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            ...
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "promote_start_date, promote_end_date, goods_img, goods_thumb, original_img, keywords, goods_brief, goods_product_tag," .
                    "seller_note, goods_weight, goods_number, warn_number, integral, give_integral, is_best, is_new, is_hot, " .
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, packing_list, customer_service, add_time, last_update, goods_type, rank_integral, suppliers_id, pinyin_keyword,group_integral,group_integral_num,is_promote_group,virtual_num,orderDay,orderHours,orderMinute,orderNum,order_dayTime,randOrder,seller_id,wheat_coin)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$is_promote','$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
                    //@author guan start
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[goods_product_tag]', '$_POST[seller_note]', '$goods_weight', '$goods_number',".
                    //@author guan end
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$_POST[goods_desc]', '$_POST[packing_list]', '$_POST[customer_service]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$rank_integral', '$suppliers_id','$pinyin','$group_integral','$group_integral_num','$_POST[is_promote_group]','$virtual_num','$orderDay','$orderHours','$orderMinute','$orderNum','$order_dayTime','$randOrder','".$_SESSION['seller_id']."',$wheat_coin)";
        }
        else
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, packing_list, customer_service,add_time, last_update, goods_type, extension_code, rank_integral, pinyin_keyword,group_integral,group_integral_num,is_promote_group,virtual_num,orderDay,orderHours,orderMinute,orderNum,order_dayTime,randOrder,seller_id,wheat_coin)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$is_promote','$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
                    //@author guan start
                    ...
                    " '$_POST[goods_desc]', '$_POST[packing_list]', '$_POST[customer_service]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$code', '$rank_integral','$pinyin','$group_integral','$group_integral_num','$_POST[is_promote_group]','$virtual_num','$orderDay','$orderHours','$orderMinute','$orderNum','$order_dayTime','$randOrder','".$_SESSION['seller_id']."',wheat_coin)";
        }
    }
    else
    {
        ...
        $sql = "UPDATE " . $ecs->table('goods') . " SET " .
				...
				"group_integral = '$group_integral', " . //ecmoban模板堂 --zhuo
				"group_integral_num = '$group_integral_num', " . //ecmoban模板堂 --zhuo 
				"is_promote_group = '$_POST[is_promote_group]', " . //ecmoban模板堂 --zhuo
				"virtual_num = '$virtual_num', " . //ecmoban模板堂 --zhuo
				"orderDay = '$orderDay', " . //ecmoban模板堂 --zhuo 2013-12-3
				"orderHours = '$orderHours', " . //ecmoban模板堂 --zhuo 2013-12-3
				"orderMinute = '$orderMinute', " . //ecmoban模板堂 --zhuo 2013-12-3
				"orderNum = '$orderNum', " . //ecmoban模板堂 --zhuo 2013-12-3 
				"order_dayTime = '$order_dayTime', " . //ecmoban模板堂 --zhuo 2013-12-3
				"randOrder = '$randOrder', " . //ecmoban模板堂 --zhuo 2013-12-3
    ...
    else
    {
        ...
        $notice_list = $db->getAll($sql);
       	...
       	foreach($notice_list as $key=>$val){
			...
			//短信发送
       		if($shop_price <= $val['hopeDiscount'] && $val['cellphone'] && $_CFG['sms_send'] == 1){
				include_once('../includes/cls_sms.php');
            	$sms = new sms();

				$send_type = 2;
				$res = $sms->send($val['cellphone'], sprintf($GLOBALS['_LANG']['sale_notice_sms'],$user_name,$_POST['goods_name'],$shop_price_format));
				...
				if($res){
					$sql = "UPDATE " .$ecs->table('sale_notice'). " SET status = 1, send_type=2 WHERE goods_id = '$_REQUEST[goods_id]' AND user_id='$val[user_id]'";
				 	$db->query($sql);
			        $send_ok = 1;
				    notice_log($_POST['goods_name'],$val['cellphone'],$send_ok,$send_type);
				}else{
       		...
       		if($send_ok = 0 && $shop_price <= $val['hopeDiscount'] && $val['email']){
		        ...
		        if (send_mail($user_name, $val['email'], $template['template_subject'], $content, $template['is_html']))
		        {
		        	$sql = "UPDATE " .$ecs->table('sale_notice'). " SET status = 1, send_type=1 WHERE goods_id = '$_REQUEST[goods_id]' AND user_id='$val[user_id]'";
		   		 	$db->query($sql);
		            $send_ok = 1;
		            notice_log($_POST['goods_name'],$val['email'],$send_ok,$send_type);
		        }
		        else
		        {
...
//ecmoban模板堂 --zhuo start 2013-12-3
elseif ($_REQUEST['act'] == 'order_dayTime')
{
    check_authz_json('goods_manage');

    $goods_id = intval($_REQUEST['goods_id']);
    $orderDay = !empty($_REQUEST['orderDay']) ? intval($_REQUEST['orderDay']) : 0;
	$orderHours = !empty($_REQUEST['orderHours']) ? intval($_REQUEST['orderHours']) : 0;
	$orderMinute = !empty($_REQUEST['orderMinute']) ? intval($_REQUEST['orderMinute']) : 0;
	$orderNum = !empty($_REQUEST['orderNum']) ? intval($_REQUEST['orderNum']) : 0;
    ...
    if($order_dayTime)
    {
		$sql="update ". $ecs->table('goods')." set order_dayTime = '$order_dayTime' WHERE goods_id='$goods_id'";
		$db->query($sql);
		
		/*$sql="update ". $ecs->table('order_goods')." set virtual_gmunber = virtual_gmunber + '$orderNum' WHERE goods_id='$goods_id'";
		$db->query($sql);*/
		
        $sql="SELECT order_dayTime FROM ". $ecs->table('goods')." WHERE goods_id='$goods_id'";
		$order_dayTime = $db->getOne($sql);
		$order_dayTime    = local_date('Y-m-d H:i:s', $order_dayTime);
		$order_dayTime = str_replace('-','/',$order_dayTime);
    }
	
	$_SESSION['order_dayTime'] = $order_dayTime;
	
    make_json_result($order_dayTime);
}
//ecmoban模板堂 --zhuo end 2013-12-3
elseif ($_REQUEST['act'] == 'check_products_goods_sn')
{
