...
//获得会员所拥有的有麦币
$user_wheat_coin=$db->getOne("select wheat_coin from ".$ecs->table('users')." where user_id='".$_SESSION['user_id']."'");
$smarty->assign('user_wheat_coin', $user_wheat_coin);
...
$act_arr=array('checkout','done','select_shipping','select_pack','select_payment','select_insure','select_card','validate_bonus','use_coin','change_surplus');
//by wang
if(!in_array($_REQUEST['step'],$act_arr))
{
	$_SESSION['total']='';
	$_SESSION['seller_total']='';
}
...
if ($_REQUEST['step'] == 'add_to_cart')
{
	...
	/*-------wang用户白条--------*/
	//判断是否为用户白条客户和是否可以下单
	$bt_sql="select b.*,bl.repay_date from ".$GLOBALS['ecs']->table('baitiao')." as b left join ".$GLOBALS['ecs']->table('baitiao_log')." as bl on b.baitiao_id=bl.baitiao_id where bl.is_repay=0 and b.user_id='".$_SESSION['user_id']."'";
	$bt_info=$GLOBALS['db']->getAll($bt_sql);
	if($bt_info)
	{
		$over_date=gmtime()-(($bt_info[0]['over_repay_trem'])*24*3600);
		foreach($bt_info as $val)
		{
			if($over_date>=$val['repay_date'])
			{
				$result['error'] = 1;
				$result['message']="您有过期未付清的用户白条款额,您已被禁止下单,请先付清欠款";
...
elseif ($_REQUEST['step'] == 'checkout')
{
	...
	$smarty->assign('wheat_coin_v',$_CFG['wheat_coin']);//系统设置的有麦币实际人民币价值
	...
	foreach($cart_goods as $key=>$val)
	{
		...
		$seller_cart[$key]['shippings']=seller_available_shipping_list($region,$key);
		$seller_cart[$key]['cart_weight_price'] =seller_cart_weight_price($flow_type,$key);
		$seller_cart[$key]['goods_list']=$val;
		$seller_cart[$key]['wheat_coin']=$val[0]['wheat_coin']?$val[0]['wheat_coin']:0;
	}	
	...
	foreach($cart_goods as $key=>$val)
	{
		$seller_total[$key]['total']=seller_order_fee($order, $val, $consignee);
	}
	if(empty($_SESSION['seller_total']))
	{
		$_SESSION['seller_total']=$seller_total;
	}
	...
	if(empty($_SESSION['total']))
	{
		...
		$total = seller_order_fee($order, $cart_goods, $consignee);

		$_SESSION['total']=$total;
		$_SESSION['total']['all_amount']=$total['amount'];
	...
	foreach($seller_cart as $key=>$val)
	{
		// 查看购物车中是否全为免运费商品，若是则把运费赋为零
		$sql = 'SELECT count(*) FROM ' . $ecs->table('cart') . " WHERE `session_id` = '" . SESS_ID. "' AND `extension_code` != 'package_buy' AND `is_shipping` = 0 and seller_id='$key'";
		$shipping_count = $db->getOne($sql);
	
		foreach ($val['shippings'] AS $skey => $sval)
		{
			$shipping_cfg = unserialize_config($sval['configure']);
			$shipping_fee = ($shipping_count == 0 AND $sval['cart_weight_price']['free_shipping'] == 1) ? 0 : shipping_fee($sval['shipping_code'], unserialize($sval['configure']),
			$sval['cart_weight_price']['weight'], $sval['cart_weight_price']['amount'], $sval['cart_weight_price']['number']);
	
			$seller_cart[$key]['shippings'][$skey]['format_shipping_fee'] = price_format($shipping_fee, false);
			$seller_cart[$key]['shippings'][$skey]['shipping_fee']        = $shipping_fee;
			$seller_cart[$key]['shippings'][$skey]['free_money']          = price_format($shipping_cfg['free_money'], false);
			$seller_cart[$key]['shippings'][$skey]['insure_formated']     = strpos($sval['insure'], '%') === false ?
				price_format($sval['insure'], false) : $sval['insure'];
	
			/* 当前的配送方式是否支持保价 */
			$seller_cart[$key]['insure_disabled'] = ($sval['insure'] == 0);
			$seller_cart[$key]['cod_disabled']    = ($sval['support_cod'] == 0);
		}
	}
...
elseif ($_REQUEST['step'] == 'select_shipping')
{
    ...
    else
    {
		...
		if(!empty($_SESSION['seller_total'][$seller_id]['sub_coin']))
		{
			$seller_sub_coin=$_SESSION['seller_total'][$seller_id]['sub_coin'];
		}
		
		$_SESSION['seller_total'][$seller_id]=order_fee($order, $cart_sup_goods[$seller_id], $consignee);
		$_SESSION['seller_total'][$seller_id]['sub_coin']=$seller_sub_coin;
		$_SESSION['seller_total'][$seller_id]['shipping_id']=$order['shipping_id'];

		$_SESSION['total']['shipping_fee']=0;
		foreach($_SESSION['seller_total'] AS $key=>$val)
		{
			$_SESSION['total']['shipping_fee']=intval($_SESSION['total']['shipping_fee'])+$val['shipping_fee'];
		}
		$order_amount=$_SESSION['total']['amount']+$_SESSION['total']['shipping_fee']+$_SESSION['total']['pay_fee']-$_SESSION['total']['sub_coin'];
		$_SESSION['total']['amount_formated']=price_format($order_amount, false);
		$_SESSION['total']['all_amount']=$order_amount;
		$_SESSION['total']['shipping_totelfee_formated']=price_format($_SESSION['total']['shipping_fee'], false);
        $smarty->assign('total',$_SESSION['total']);
		//by wang 
...
elseif ($_REQUEST['step'] == 'select_insure')
{
    ...
    else
    {
        ...
        $order = seller_flow_order_info();
		$order['shipping_id']=$_SESSION['seller_total'][$seller_id]['shipping_id'];
		...
		$_SESSION['seller_total'][$seller_id]['sub_coin']=$seller_sub_coin;
		$_SESSION['seller_total'][$seller_id]['shipping_id']=$order['shipping_id'];
		$_SESSION['seller_total'][$seller_id]['need_insure']=intval($order['need_insure']);

		$_SESSION['total']['shipping_insure']=0;
		foreach($_SESSION['seller_total'] AS $key=>$val)
		{
			$_SESSION['total']['shipping_insure']=intval($_SESSION['total']['shipping_insure'])+$val['shipping_insure'];
		}

		$order_amount=$_SESSION['total']['amount']+$_SESSION['total']['shipping_fee']+$_SESSION['total']['shipping_insure']+$_SESSION['total']['pay_fee']-$_SESSION['total']['sub_coin'];
		$_SESSION['total']['amount_formated']=price_format($order_amount, false);
		$_SESSION['total']['all_amount']=$order_amount;
		$_SESSION['total']['shipping_insure_formated']=price_format($_SESSION['total']['shipping_insure'], false);
...
elseif ($_REQUEST['step'] == 'use_coin')//有麦币的操作
{
		$seller_id=intval($_REQUEST['seller_id']);
		$sub_coin=floatval($_REQUEST['sub_coin']);
    ...
    else
    {
        ...
        $order = seller_flow_order_info();
		$order['shipping_id']=$_SESSION['seller_total'][$seller_id]['shipping_id'];
        $order['need_insure'] = $_SESSION['seller_total'][$seller_id]['need_insure'];
		...
		$_SESSION['seller_total'][$seller_id]['shipping_id']=$order['shipping_id'];
		$_SESSION['seller_total'][$seller_id]['sub_coin']=$sub_coin;
		
		$_SESSION['total']['sub_coin']=0;
		foreach($cart_sup_goods as $key=>$val)
		{
			//使用的有麦币不可以超过所拥有的有麦币
			if((($_SESSION['total']['sub_coin']+$_SESSION['seller_total'][$key]['sub_coin'])/$_CFG['wheat_coin'])>$user_wheat_coin)
			{
				$result['error']="no_coin";
				
			}
			else
			{
				$_SESSION['total']['sub_coin']+=$_SESSION['seller_total'][$key]['sub_coin'];
			}
...
elseif ($_REQUEST['step'] == 'select_payment')
{
    ...
    else
    {
		...
		foreach($cart_sup_goods as $key=>$val)
		{
			...
			$order['shipping_id']=$_SESSION['seller_total'][$key]['shipping_id'];
			$order['need_insure'] = $_SESSION['seller_total'][$key]['need_insure'];
			if(!empty($_SESSION['seller_total'][$key]['sub_coin']))
			{
				$seller_sub_coin=$_SESSION['seller_total'][$key]['sub_coin'];
			}
		
			$_SESSION['seller_total'][$key]=order_fee($order, $val, $consignee);
			$_SESSION['seller_total'][$key]['sub_coin']=$seller_sub_coin;
			$_SESSION['seller_total'][$key]['shipping_id']=$order['shipping_id'];
			$_SESSION['seller_total'][$key]['need_insure']=$order['need_insure'];
			$_SESSION['seller_total'][$key]['pay_id']=$order['pay_id'];

		}
		
		$_SESSION['total']['pay_fee']=0;
		foreach($_SESSION['seller_total'] AS $key=>$val)
		{
			$_SESSION['total']['pay_fee']=intval($_SESSION['total']['pay_fee'])+$val['pay_fee'];
		}
		$order_amount=$_SESSION['total']['amount']+$_SESSION['total']['shipping_fee']+$_SESSION['total']['shipping_insure']+$_SESSION['total']['pay_fee']-$_SESSION['total']['sub_coin'];
		$_SESSION['total']['amount_formated']=price_format($order_amount, false);
		$_SESSION['total']['all_amount']=$order_amount;
		$_SESSION['total']['pay_fee_formated']=price_format($_SESSION['total']['pay_fee'], false);
...
elseif ($_REQUEST['step'] == 'select_pack')
{
    ...
    else
    {
        ...
        $total = seller_order_fee($order, $cart_goods, $consignee,count($cart_sup_goods));
        $smarty->assign('total', $total);
...
elseif ($_REQUEST['step'] == 'change_surplus')
{
    ...
    else
    {
        ...
        else
        {
            ...
            /* 计算订单的费用 */
            if(isset($surplus)&&$surplus>$_SESSION['total']['all_amount'])
			{
				$_SESSION['total']['all_amount']=0;	
			}
			else
			{
				$_SESSION['total']['all_amount']-=$surplus;
			}
			$_SESSION['total']['amount_formated']=price_format($_SESSION['total']['all_amount'], false);
...
elseif ($_REQUEST['step'] == 'done')
{
    ...
    $order = array(
		...
		'pay_code'     => $_POST['alipay_bank'],//wang 支付宝网银直连
   ...
   //wang 商家入驻
	...
	foreach($cart_seller_goods as $seller_id=>$cart_goods)
	{
		...
		$seller_order=$_SESSION['seller_total'][$seller_id];

		if(isset($is_real_good))
		{
			$sql="SELECT shipping_id FROM " . $ecs->table('shipping') . " WHERE shipping_id=".$seller_order['shipping_id'] ." AND enabled =1"; 
			if(!$db->getOne($sql))
			{
		...
		$order['shipping_id']=$seller_order['shipping_id'];
		$order['need_insure']=$seller_order['shipping_insure']>0?1:0;
		$total = seller_order_fee($order, $cart_goods, $consignee);
		...
		$order['tax']          = $total['tax'];
		$order['wheat_coin']=$seller_order['sub_coin']?intval($seller_order['sub_coin'])/$_CFG['wheat_coin']:0;
		$total['amount']=$total['amount']-$order['wheat_coin'];
		...
		{		
			if(!$bt_info)
			{
				show_message('您未被授权使用用户白条支付，请选择其他支付方式','返回上一页','flow.php?step=checkout');
			}
			$remain_amount=floatval($bt_info['amount'])-floatval($repay_bt);
			if($order['order_amount']>$remain_amount)
			{
				show_message('用户白条余额不足，请选择其他支付方式','返回上一页','flow.php?step=checkout');
			}
			else
			{
				//还款到期时间
				$over_sql="select count(log_id) from ".$ecs->table('baitiao_log')." where is_repay=0 and repay_date<=".gmtime()." and user_id='$user_id'";
				$is_over_date=$db->getOne($over_sql);
				if($is_over_date>0)
				{
					show_message('您有过期未付款金融，不可以用用户白条支付，请选择其他支付方式','返回上一页','flow.php?step=checkout');
				}
				else
				{
					$order['order_status'] = OS_CONFIRMED;
		...
		/*----用户白条----*/
		//记录用户白条消费记录
		if($new_order_id>0&&$order['pay_id']==100)
		{
			$repay_date=gmtime()+($bt_info['repay_term']*24*3600);
			$insert_bt_log="insert into ".$ecs->table('baitiao_log')." (baitiao_id,user_id,use_date,repay_date,order_id,is_repay) values (".$bt_info['baitiao_id'].",'$user_id',".gmtime().",'$repay_date','$new_order_id',0)";	
		...
		//跟新用户有麦币by wang
		if($order['wheat_coin']>0)
		{
			$u_coin_sql="update ".$GLOBALS['ecs']->table('users')." set wheat_coin=(wheat_coin-".$order['wheat_coin'].") where user_id='$user_id'";
			echo $u_coin_sql;
			$GLOBALS['db']->query($u_coin_sql);
		}
		/* 如果使用库存，且下订单时减库存，则减少库存 */
		if ($_CFG['use_storage'] == '1' && $_CFG['stock_dec_time'] == SDT_PLACE)
		{
	...
	/* 取得支付信息，生成支付代码 */
			if ($order['order_amount'] > 0)
			{
				...
				{
					include_once('includes/modules/payment/alipay_payment/' . $payment['pay_code'] . '.php');
