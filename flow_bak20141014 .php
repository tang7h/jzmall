   ...
   //wang 商家入驻
	...
	foreach($cart_seller_goods as $seller_id=>$cart_goods)
	{
		...
		{		
			...
			{
				//还款到期时间
				$over_sql="select count(log_id) from ".$ecs->table('baitiao_log')." where is_repay=0 and repay_date<=".gmtime();
		...
		$order['log_id'] = insert_pay_log($new_order_id, $order['order_amount'], PAY_ORDER);
			...
			if ($order['order_amount'] > 0)
			{
				...
				$pay_online[$seller_id] = "<div style='display:inline-block;' >".$pay_obj->get_code($order, unserialize_config($payment['pay_config']))."</div>";
