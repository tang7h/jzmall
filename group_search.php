 ...
 * ============================================================================
 * $Author: liubo $
 * $Id: group_search.php 17217 2011-01-19 06:29:08Z liubo $
*/
...
if (empty($_GET['encode']))
{
    ...
    header("Location: group_search.php?encode=$string\n");
...
if ($_REQUEST['act'] == 'advanced_search')
{
	...
	$smarty->assign('id_nav',       get_proup_class_nav($cat_id));   //获得团购导航位置  ecmoban模板堂 --zhuo
...
{	
    ...
    $sort = (isset($_REQUEST['sort'])  && in_array(trim(strtolower($_REQUEST['sort'])), array('goods_id', 'shop_price', 'last_update', 'goods_number','promote_price','new_order'))) ? trim($_REQUEST['sort'])  : $default_sort_order_type;
    ...
    /* 获得符合条件的商品总数 */  //ecmoban模板堂 --zhuo 增加了 AND g.is_promote = 1 ---$time = gmtime();    -- AND g.promote_end_date >= ".$time."
	$cat_id = get_fine_group_cat($_REQUEST['category']);	
	...
	if($sort == 'goods_number'){
		...
		$sql   = "SELECT COUNT(*) FROM " .$ecs->table('goods'). " AS g ".
				"WHERE g.is_delete = 0 AND g.is_on_sale = 1 and g.is_display=1  AND g.is_promote = 1 AND g.promote_end_date >= ".$time." AND g.is_alone_sale = 1 $attr_in ".
    ...
    /* 查询商品 */ //ecmoban模板堂 --zhuo   增加g.goods_number as g_num ------AND g.is_promote = 1---$time = gmtime();-- AND g.promote_end_date >= ".$time."
	...
	if($sort == 'goods_number'){
		$sql = "SELECT sum( og.goods_number ) + g.virtual_num AS goods_number, g.goods_id, g.goods_name, g.goods_number as g_num, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ".
            ...
            "WHERE g.is_delete = 0 and g.is_display=1  AND g.is_on_sale = 1 AND g.is_promote = 1 AND  oi.pay_status =2 AND g.promote_end_date >= ".$time." AND g.is_alone_sale = 1 $attr_in ".
                "AND (( 1 " . $categories . $keywords . $brand . $min_price . $max_price . $intro . $outstock . " ) ".$tag_where." ) and " . $children . 
            " and  g.promote_end_date >= ".$time." GROUP BY og.goods_name ORDER BY $sort $order";
			
	}elseif($sort == 'new_order'){
		
		$sql = "SELECT oi.pay_time AS new_order, g.goods_id, g.goods_name, g.goods_number as g_num, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ".
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
                "g.promote_price, g.promote_start_date, g.promote_end_date, g.goods_thumb, g.goods_img, g.goods_brief, g.goods_type ".
            "FROM " .$ecs->table('goods'). " AS g ".
            "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
            ...
            "WHERE g.is_delete = 0 and g.is_display=1 AND g.is_on_sale = 1 AND g.is_promote = 1 AND  oi.pay_status =2 AND oi.pay_time >= ". $firstSecToday ." and oi.pay_time <= ". $lastSecToday ."  AND g.promote_end_date >= ".$time." AND g.is_alone_sale = 1 $attr_in ".
                "AND (( 1 " . $categories . $keywords . $brand . $min_price . $max_price . $intro . $outstock . " ) ".$tag_where." ) and " . $children . 
            " and  g.promote_end_date >= ".$time." GROUP BY og.goods_name ORDER BY $sort $order";

	}else{
		$sql = "SELECT g.goods_id, g.goods_name, g.goods_number as g_num, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ".
					"IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
					"g.promote_price, g.promote_start_date, g.promote_end_date, g.goods_thumb, g.goods_img, g.goods_brief, g.goods_type ".
				"FROM " .$ecs->table('goods'). " AS g ".
				"LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
						"ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
				"WHERE g.is_delete = 0 and g.is_display=1 AND g.is_on_sale = 1 AND g.is_promote = 1 AND g.promote_end_date >= ".$time." AND g.is_alone_sale = 1 $attr_in ".
					"AND (( 1 " . $categories . $keywords . $brand . $min_price . $max_price . $intro . $outstock . " ) ".$tag_where." ) " .
				"ORDER BY $sort $order";	
    ...
    while ($row = $db->FetchRow($res))
    {
		...
		$arr[$row['goods_id']]['total'] 		 = get_group_cat_goods_count($row['goods_id']);  //购买人数 
		$arr[$row['goods_id']]['g_num']          = $row['g_num']; //ecmoban模板堂 --zhuo
    }
    ...
    /* 分页 */
    $url_format = "group_search.php?category=$category&amp;keywords=" . urlencode(stripslashes($_REQUEST['keywords'])) . "&amp;brand=" . $_REQUEST['brand']."&amp;action=".$action."&amp;goods_type=" . $_REQUEST['goods_type'] . "&amp;sc_ds=" . $_REQUEST['sc_ds'];
    if (!empty($intromode))
    {
    ...
    $pager = get_pager('group_search.php', $pager['search'], $count, $page, $size);
    $pager['display'] = $display;

    $smarty->assign('url_format', $url_format);
    $smarty->assign('pager', $pager);
	//ecmoban模板堂 --zhuo 分页 start
	...
	$smarty->assign('pager_seach', $pager['search']);
	$smarty->assign('script_name', 'group_search');
