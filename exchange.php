...
elseif ($_REQUEST['act'] == 'view')
{
    ...
    if (!$smarty->is_cached('exchange_goods.dwt', $cache_id))
    {
        ...
        else
        {
            ...
            /* 上一个商品下一个商品 */
            $sql = "SELECT eg.goods_id FROM " .$ecs->table('exchange_goods'). " AS eg," . $GLOBALS['ecs']->table('goods') . " AS g WHERE eg.goods_id = g.goods_id AND eg.goods_id > " . $goods['goods_id'] . " AND eg.is_exchange = 1 AND g.is_delete = 0  and g.is_display=1 LIMIT 1";
            ...
            $sql = "SELECT max(eg.goods_id) FROM " . $ecs->table('exchange_goods') . " AS eg," . $GLOBALS['ecs']->table('goods') . " AS g WHERE eg.goods_id = g.goods_id AND eg.goods_id < ".$goods['goods_id'] . " AND eg.is_exchange = 1 AND g.is_delete = 0 and g.is_display=1 ";
...
function exchange_get_goods($children, $min, $max, $ext, $size, $page, $sort, $order)
{
    ...
    $where = "eg.is_exchange = 1 AND g.is_delete = 0 and g.is_display=1  AND ".
...
function get_exchange_recommend_goods($type = '', $cats = '', $min =0,  $max = 0, $ext='')
{
    ...
    $sql =  'SELECT g.goods_id, g.goods_name, g.goods_name_style, eg.exchange_integral, ' .
            ...
            'WHERE eg.is_exchange = 1 AND g.is_delete = 0  and g.is_display=1 ' . $price_where . $ext;
    $num = 0;
